...
\begin{equation*}
    \E_T[R_i] = \bm{\hat{\beta}}_{i}^{'} \bm{\lambda} + \alpha_i \inlinesep \forall i=1,2,\dots,N
\end{equation*}
...
\begin{align*}
    \sigma^2(\hat{\lambda}_k) &= \frac{1}{T^2} \sum_{t=1}^{T} (\hat{\lambda}_{k,t} - \hat{\lambda}_k)^2 \\
    \sigma^2(\hat{\alpha}_i) &= \frac{1}{T^2} \sum_{t=1}^{T} (\hat{\alpha}_{i,t} - \hat{\alpha}_i)^2
\end{align*}
