...
其次，当计算完日频或月频的夏普比率后，需要将其转化为年化的夏普比率。当收益率为日频，其均值乘以252交易日（或对于中国市场240交易日），得到年化收益率均值，而对于月频收益率则乘以12，但需要注意分子波动率却不随时间线性变化，而是随着时间的平方根变化。对于样本标准差而言$\sigma = \sqrt{\frac{1}{N-1} \sum_{i=1}^{n} (r_i - \bar{r})^2}$，当$r_i$由日频放大为年频，在根号内部标准差的分母部分应乘以$252^2$，但其样本数量减少$252$倍，因此标准差需乘以$\sqrt{252}$。因此对于日频的夏普比率，应乘以$\frac{252}{\sqrt{252}} = \sqrt{252}$进行转换，而对于月频则同理应乘以$\sqrt{12}$进行转换。还需要注意的是偏度和峰度不随着放大缩小改变，即不受年化的影响。注意：日频或月频的夏普比率转换为年频，与实际年频计算存在一定差距。

\begin{example}
    对于一个10年的日频收益率（$r_i$）样本，有$5\times 252$个观察值，若将其转化为年频数据，则只有10个观察值。对于日频收益率，其波动率应有：
    \begin{equation*}
        \sigma_{daily} = \sqrt{\frac{1}{10 \times 252 - 1} \sum(r_i -\bar{r})^2}
    \end{equation*}

    而对于年频收益率：
    \begin{equation*}
        \sigma_{annual} = \sqrt{\frac{1}{10 - 1} \sum 252^2 (r_i -\bar{r})^2} \approx \sqrt{252} \sigma_{daily}
    \end{equation*}
\end{example}
