...
可以看到\textbf{连续复利收益率}或\textbf{对数收益率}服从期望值为$(\mu - \tfrac{\sigma^2}{2})dt$，方差为$\sigma^2 dt$的\textbf{正态分布}，与现实较为吻合。且$d\ln S_t$的定义，使得股票价格非负。并且在$T$时刻，\textbf{股票价格的对数}，也服从正态分布：
\begin{equation*}
...
\toprule
\multicolumn{1}{l}{}
& \multicolumn{1}{c}{\textbf{百分比}} & \multicolumn{1}{c}{\textbf{对数}} \\
\midrule
\multirow{1}{*}{\textbf{单期收益}} 
& $R_{pct} = \frac{V_T-V_t}{V_t}$
...
由几何布朗运动的推导可知，在\textbf{对数收益率}或\textbf{连续复利收益率}中，$d\ln S$（极短时间内）和$\Delta \ln S$（较长时间内）都服从正态分布：
\begin{align*}
    d\ln S &= \left( \mu - \frac{1}{2}\sigma^2\right) dt + \sigma dZ_t \sim N \left( (\mu-\frac{\sigma^2}{2})dt, \sigma^2 dt \right) \\
...
??????可以发现，在极短时间间距内$dS_t/S_t$服从正态分布。而在较长时间间距内,因$S_t$的大小改变???，使得其均值和方差发生改变，而再不服从正态分布。
...
若使用对数收益率（模型），其期望为$\mu-\frac{\sigma^2}{2}$，即算术平均$\mu$需要减去$\frac{\sigma^2}{2}$。在此例子中百分比收益率均值为0，样本方差为$\frac{1}{3}$，此时对数收益率的期望为$-\frac{1}{6}\approx -16.67\%$。即波动越大，降低实际收益率，更符合现实情况，贴近几何平均收益率，具有经济学意义。计算实际对数收益率的算术平均为$2(\ln 1.5 + \ln 0.5)/4 = -14.38\%$。
