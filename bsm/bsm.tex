...
\begin{itemize}
    ...
    \item 平稳性：增量$\Delta Z$服从均值为零、方差等于时间长度的正态分布，即$\Delta Z_i  \sim N(0,\Delta t_i)$
\end{itemize}
...
\subsection{二次变差（Quadratic variation）}

考虑区间$[0,T]$，定义$\Pi=\{0=t_0<t_1<t_2<\dots<t_N=T\}$和最大步幅$\left\Vert\Pi \right\Vert = \max\limits_{i}(t_{i+1}-t_{i})$。为方便后续推导，假设步幅相同，即$t_{i+1} - t_{i} = \tfrac{T}{n}$。$s_i \in (t_i,t_{i+1})$,则有对于任意一个连续函数$f(t)$，其二次变差定义为：
\begin{equation*}
    \sum^{N-1}_{i=0} \left[ f(t_{i+1} - f(t_i)) \right]^2
\end{equation*}

对于一个连续且在$[0,T]$中处处可微的函数$f(t)$。利用微分中值定理，可以看到随着时间的越来越细的划分，$\left\Vert\Pi \right\Vert$趋近于零，则应有处处可微的连续函数，在区间$[0,T]$的二次变分为0。
\begin{align*}
    \sum_i \left[ f(t_{i+1}) -f(t_{i}) \right]^2 
    &= \sum_i f'(s_i)^2 (t_{i+1} - t_{i})^2 \\
    &\leq \max_{s\in [0,T]}f'(s)^2 \cdot \sum_i (t_{i+1} -t_{i})^2 \\
    &\leq \max_{s\in [0,T]}f'(s)^2 \cdot \max\limits_{i}(t_{i+1} -t_{i})\cdot T
\end{align*}

而对于布朗运动$B(t)$，虽然连续，但处处不可微分：
对于Brown运动，其非零的二次变分说明随机性使得它的波动太频繁，以至于不管我们如何细分区间、得到多么微小的划分区间T，这些微小区间上的位移差的平方逐段累加起来的总和(二次变分的几何意义)都不会消失（即二次变分不为0），而是等于这个区间的长度T

布朗运动的二次变差可写为$(dB)^2=dt$

\begin{thm}
    设$\Pi=\{0=t_0<t_1<t_2<\dots<t_N=T\}$为$[0,T]$的分划。则对应的样本二次变差有：
    \begin{align*}
        Q_{\Pi} = \sum_{i=0}^{N-1} \left[ B(t_{i+1}) - B(t_{i}) \right]^2 = T \qquad \text{almost surely.}
    \end{align*}
\end{thm}
...
\begin{proof}
    ...
    \begin{equation*}
        \E [(B(t_{i+1}) - B(t_{i}))^2] = \Var [B(t_{i+1}) - B(t_{i})] + \E^2 [B(t_{i+1}) - B(t_{i})]= t_{i+1} - t_{i}
    \end{equation*}

    由此可以得到：
    \begin{equation*}
        \lim\limits_{\left\Vert\Pi\right\Vert \rightarrow 0} \E(Q_{\Pi})
        = \sum_{i=0}^{N-1} \E \left[(B(t_{i+1}) - B(t_{i}))^2 \right]
        =  \sum_{i=0}^{N-1} (t_{i+1} - t_{i}) = T
    \end{equation*}

    对于方差，应有：
    \begin{equation*}
        \Var [(B(t_{i+1}) - B(t_{i}))^2] = \E [(B(t_{i+1}) - B(t_{i}))^4] - \E^2 [(B(t_{i+1}) - B(t_{i}))^2]
    \end{equation*}

    对于四次方项$\E [(B(t_{i+1}) - B(t_{i}))^4]$，可通过对正态分布矩母函数$M_X(t)$对$t$不断求导得到其期望，需求四阶导数$M_X^{(4)}(t)$：
    \begin{align*}
        ...
        M^{(4)}_X(t) &= (\mu^4 + 4 \mu^3 \sigma^2 t + 6 \mu^2 \sigma^4 t^2 + 6\mu^2 \sigma^2 + 4\mu\sigma^6 t^3 + 12\mu\sigma^4 t + \sigma^8 t^4 + 6\sigma^6 t^2 + 3\sigma^4)e^{\mu t +\frac{1}{2} \sigma^2 t^2}
    \end{align*}

    因为$B(t_{i+1}) - B(t_{i}) \sim N(0,t_{i+1}-t_{i})$，其均值$\mu=0$计算四阶中心矩$M_X^{(4)}(0) = 3\sigma^2$，令$t=0$：
    \begin{equation*}
        \E [(B(t_{i+1}) - B(t_{i}))^4] = 3(t_{i+1}-t_{i})^2 
    \end{equation*}
    ...
    因此在$[0,T]$的方差为：
    \begin{align*}
        \Var(Q_{\Pi}) &= \sum_{i=0}^{N-1} \Var \left[(B(t_{i+1}) - B(t_{i}))^2 \right] \\
        &=  \sum_{i=0}^{N-1} 2(t_{i+1}-t_{i})^2
        \leq \sum_{i=0}^{N-1} 2\left\Vert \Pi \right\Vert (t_{i+1}-t_{i}) \\
        &= 2\left\Vert \Pi \right\Vert \sum_{i=0}^{N-1} (t_{i+1}-t_{i}) \\
        &= 2\left\Vert \Pi \right\Vert T
    \end{align*}

    由此可知当最大步幅$||\Pi||$趋近于零时，方差也将趋近于零：
    \begin{align*}
        \lim\limits_{\left\Vert\Pi\right\Vert \rightarrow 0} \Var(Q_{\Pi}) = 0
    \end{align*}

    利用切比雪夫不等式（Chebyshev's inequality）：
    \begin{align*}
        P(\left\vert Q_{\Pi} - \E(Q_{\Pi}) \right\vert &\geq \frac{1}{n}) \leq n^2 \Var(Q_{\Pi}) \\
        \lim_{\left\Vert\Pi\right\Vert \rightarrow 0} P(\left\vert Q_{\Pi} - T ) \right\vert &\geq \frac{1}{n}) \leq n^2 \lim_{\left\Vert\Pi\right\Vert \rightarrow 0} \Var(Q_{\Pi})  \\
        \lim_{n \rightarrow \infty} P(\left\vert Q_{\Pi} - T ) \right\vert &\geq \frac{1}{n}) = 0
    \end{align*}
    
    因此在概率上几乎必然成立，即虽然有可能存在某些布朗运动使得其二次变差为T不成立，但这样的情形概率为0:
    \begin{equation*}
        \lim_{\left\Vert\Pi\right\Vert \rightarrow 0} Q_{\Pi} = \E Q_\Pi = T
    \end{equation*}
...
\textbf{随机微分方程}（Stochastic differential equation，SDE）指的是包含随机过程的微分方程。
...
历史波动率是基于历史信息得到的，指资产收益率在过去一段时间内表现出的波动水平，可以由资产收益率在过去一段时间内的标准差计算得到。一般使用日频数据，如计算：过去30个交易日的标准差。

【核实】在 Black Scholes 的框架下，也就是假设股票价格服从 GBM 的时候，historical volatility 可以用 quadratic variation 计算。如下式：
\begin{equation*}
...
已实现波动率是针对频率较高的数据计算的一种波动率，又称为日内波动率或高频波动率。高频数据是指以小时、分钟或秒为采集频率的数据。一般使用日内高频频率为5分钟。为：
\begin{equation*}
    \text{RV}_t = \sum^{N}_{j=1} r^2_{t,j}
\end{equation*}
...
假定市场上的期权或者权证的交易价格满足Black-Scholes-Merton（BSM）期权定价公式，将市场上可以观测到的标的资产价格（$S$）、执行价格（$K$）、利率（$r$）、期限（$\tau$）作为已知变量代入定价公式中，则可以得到期权当前的市场价格所隐含的波动率，此时提取的为期限内的波动率（Option-implied volatility expectations until expiration），为未来波动率的预期。
\begin{align*}
...
一个方差互换合约的Payoff定义如下：其中第一项为已实现波动率，而第二项$X_{var}$为使得合约在签订之时，合约价值为0的用于交换的固定方差：
\begin{equation*}
...
\begin{equation*}
    e^{-rT} \tilde{\E} \left[ \frac{1}{T} \int^T_0 \sigma^2 dt - X_{var} \right] = 0 \quad \Rightarrow \quad
    X_{var} = \tilde{\E} \left[ \frac{1}{T} \int^T_0 \sigma^2_t dt \right]
\end{equation*}

在风险中性测度下，此时$\mu=r$为常数，并且根据上述对数收益率$d\ln S_t$，积分形式有：
\begin{align*}
    \int^T_0 \frac{\sigma^2_t}{2} dt 
    &= \int^T_0 r dt + \int^T_0 \sigma_t d\tilde{Z}_t -\int^T_0 d\ln S_t \\
    &= rT - \ln S_T + \ln S_0 + \int^T_0 \sigma_t d\tilde{Z}_t \\
    &= -\ln S_T + \ln S_t + \ln e^{rT} + \int^T_0 \sigma_t d\tilde{Z}_t \\
    &= - \left[ \ln S_T - \ln S_0 e^{rT} \right] + \int^T_0 \sigma_t d\tilde{Z}_t \\
    &= - \left[ \ln \frac{S_T}{S_0 e^{rT}} \right] + \int^T_0 \sigma_t d\tilde{Z}_t
\end{align*}

由于$\int^T_0 \sigma_t d\tilde{Z}_t$为随机波动项，因此取期望后为0。需要注意$S_T$为随机变量，并且有$\tilde{\E}_0(S_T)=S_0 e^{rT}=F_{0,T}$，令$f(x)=\ln x$。利用下述两种方法（带积分余项的泰勒展开式以及使用狄拉克$\delta$函数）在$K=K_0$处展开带有随机变量的$S_T$的$\ln\frac{S_T}{K_0}$项则有：
\begin{align*}
    X_{var} &= \frac{2}{T} \tilde{\E} \left[ - \ln\frac{S_T}{S_0 e^{rT}} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ - \ln\frac{S_T}{F} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[\ln\frac{F}{K_0} - \ln\frac{S_T}{K_0}  \right] \\
    &= \frac{2}{T} \tilde{\E}  \left[ \ln\frac{F}{K_0}- f'(K_0) (S_T-K_0) - \int^{K_0}_{0} f''(K) (K-S_T)^+ dK - \int^{\infty}_{K_0} f''(K) (S_T-K)^+ dK  \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \ln\frac{F}{K_0} - \frac{S_T-K_0}{K_0} + \int^{K_0}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{K_0} \frac{(S_T-K)^+}{K^2} dK \right]
\end{align*}

对于前两项，并使用泰勒级数分解$\ln \frac{F}{K_0}$，即令$f(x) = \ln(x)$，并在$\frac{F}{K_0}$处展开，应有：
\begin{align*}
    \tilde{\E} \left[ \ln\frac{F}{K_0} - \frac{S_T-K_0}{K_0} \right]
    &= -\frac{\tilde{\E}(S_T)-K_0}{K_0} + \ln \frac{F}{K_0} \\
    &= -\frac{F-K_0}{K_0} + \left[ f(1) + f'(1)\left(\frac{F}{K_0}-1\right) + \frac{1}{2}f''(1)\left(\frac{F}{K_0}-1\right)^2 + \mathcal{O}\left( \frac{F}{K_0} - 1 \right)^3 \right] \\
...
\begin{align*}
    \tilde{\E} \left[ \int^{K_0}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{K_0} \frac{(S_T-K)^+}{K^2} dK \right]
    &= \int^{K_0}_{0} \frac{\tilde{\E}(K-S_T)^+ }{K^2} dK + \int^{\infty}_{K_0} \frac{\tilde{\E}(S_T-K)^+}{K^2} dK \\
...
\begin{align*}
    \sigma^2 &= \tilde{\E} \left[ \frac{1}{T} \int^T_0 \sigma^2_t dt \right] \\
    &= \frac{2}{T} \tilde{\E} \int^T_0 \left[ \frac{d S_t}{S_t} - d \ln S_t \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \int^T_0 \frac{d S_t}{S_t} - \int^T_0 d \ln S_t \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \int^T_0 rdt + \int^T_0\sigma dZ_t  - \ln \frac{S_T}{S_0} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ rT  - \ln \frac{S_T}{S_0} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \ln S_0 e^{rT} -\ln K_0 + \ln K_0 - \ln S_T \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \ln \frac{S_0 e^{rT}}{K_0} - \ln \frac{S_T}{K_0} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \ln \frac{F}{K_0} - \ln \frac{S_T}{K_0} \right] \qquad \text{（可按上述方法进行推导）} 
\end{align*}
...
\begin{property}
    ...
    \begin{equation*}
        ...
        \begin{cases}
            ...
            0 \; & x<0
        \end{cases}
        \equiv \mathbbm{1}(x \geq 0)
    \end{equation*}
    其中$\mathbbm{1}(\cdot)$为指示函数（Indicator function）。并有狄拉克$\delta$函数为赫维赛德阶跃函数的导数，两者有如下关系：
    \begin{equation*}
...
参考Carr和Madan（1998）在《Towards a Theory of Volatility Trading》中推导。假设$f(x)$为payoff，则根据狄拉克$\delta$函数的Sifting property改写函数为积分形式，并将积分分解为两部分（$K_0$非负）。为方便后续计算，利用狄拉克$\delta$函数偶函数的性质将第一部分积分改写为$K-F$。
\begin{align*}
...
\begin{align*}
    ...
    &= \bigg[ f(K) \mathbbm{1}(F<K) \bigg]^{K_0}_{0} - \int^{K_0}_{0} f'(K) \mathbbm{1}(F<K) dK \\ 
    &= f(K_0) \mathbbm{1}(F<K_0) - \cancel{f(0)\mathbbm{1}(F<0)} - \int^{K_0}_{0} f'(K) \mathbbm{1}(F<K) dK \\
    &= f(K_0)\mathbbm{1}(F<K_0) - \int^{K_0}_{0} f'(K) \mathbbm{1}(F<K) dK
\end{align*}
...
因此有$v=-H(F-K)=-\mathbbm{1}(F -K \geq 0)=-\mathbbm{1}(F \geq K)$ ，得到：
\begin{align*}
    \int^{\infty}_{K_0} f(K)\delta(F-K)dK 
    &= \bigg[ - f(K) \mathbbm{1}(F \geq K) \bigg]^{\infty}_{K_0} + \int^{\infty}_{K_0} f'(K) \mathbbm{1}(F \geq K) dK \\
    &= - \cancel{f(\infty) \mathbbm{1}(F\geq \infty)} + f(K_0) \mathbbm{1}(F\geq K_0 ) + \int^{\infty}_{K_0} f'(K) \mathbbm{1}(F \geq K) dK \\
    &= f(K_0) \mathbbm{1}(F\geq K_0 ) + \int^{\infty}_{K_0} f'(K) \mathbbm{1}(F \geq K) dK
\end{align*}

将两部分积分代入原式整理可得：
\begin{align*}
    f(F) &=  f(K_0)\mathbbm{1}(F<K_0) + f(K_0)\mathbbm{1}(F \geq K_0)
    - \int^{K_0}_{0} f'(K) \mathbbm{1}(F<K) dK 
    + \int^{\infty}_{K_0} f'(K) \mathbbm{1}(F \geq K) dK
\end{align*}

再次使用分部积分法继续分解上式中的两项积分。对于指示函数的积分$\int^{K_0}_{0}\mathbbm{1}(F<K) dK$，即计算高度为1的矩形面积。其宽度为$F$至积分上限$K_0$，如果$F<K_0$则有其积分（面积）为$K_0-F$；如过$F>K_0$，则有积分值为0：
\begin{equation*}
    \int^{K_0}_{0}\mathbbm{1}(F<K) dK =
    \begin{cases}
        K_0 - F \; & F<K_0 \\
        0 \; & F>K_0
    \end{cases}
...
或令$u=f'(K)$，$du=f''(K)dK$，$dv=\mathbbm{1}(F<K)dK$，$v=(K-F)^+$，因此第一项积分有：
\begin{align*}
    \int^{K_0}_{0} f'(K) \mathbbm{1}(F<K) dK 
...
对于第二项积分，令$u=f'(K)$，$du=f''(K)dK$，同时需注意链式法则带来的符号改变$dv=\mathbbm{1}(F \geq K)dK$，$v=-(F-K)^+$：
\begin{align*}
    \int^{\infty}_{K_0} f'(K) \mathbbm{1}(F\geq K) dK 
...
\begin{align*}
    f(F) &= f(K_0)\mathbbm{1}(F<K_0) + f(K_0)\mathbbm{1}(F \geq K_0) \\
    &\quad - \left[ f'(K_0)(K_0 - F)^+ - \int^{K_0}_{0} f''(K) (K-F)^+ dK \right]
    + \left[ f'(K_0)(F - K_0)^+ + \int^{\infty}_{K_0} f''(K) (F-K)^+ dK \right] \\
    & = f(K_0) + f'(K_0) \left[ (F-K_0)^+ - (K_0 - F)^+ \right] + \int^{K_0}_{0} f''(K) (K-F)^+ dK + \int^{K_0}_{0} f''(K) (F-K)^+ dK \\
    & = f(K_0) + f'(K_0) (F-K_0) + \int^{K_0}_{0} f''(K) (K-F)^+ dK + \int^{\infty}_{K_0} f''(K) (F-K)^+ dK
\end{align*}
