...
\setlist{noitemsep,topsep=0em,leftmargin=4em}
...
由于$\int^T_0 \sigma_t d\tilde{Z}_t$为随机波动项，因此取期望后为0。需要注意$S_T$为随机变量，并且有$\rnE_0(S_T)=S_0 e^{rT}=F_{0,T}$，令$f(x)=\ln x$。利用下述两种方法（带积分余项的泰勒展开式以及使用狄拉克$\delta$函数）在$K=K_0$处展开带有随机变量的$S_T$的$\ln\frac{S_T}{K_0}$项则有：
\begin{align*}
...
看跌-看涨平价公式（Put-call parity）或买卖权平价关系，指具有相同的行权价与到期日的欧式看涨期权与欧式看跌期权，其价格之间存在的关系。若平价关系不成立，则意味着存在套利的空间。现实市场中存在交易成本，因此买卖权平价关系不完全成立。然而流动性较好的市场中，可以近似认为买卖权平价关系成立。
...
在中国市场中，若ETF成分股分红，分红会留存在ETF之内，因此ETF期权有红利保护，即ETF价格中已经反应了红利。而对于股指期货而言，股指仅根据股票价格计算得出，成分股分红，股票价格与股指自动回落。因此股指期货无红利保护，需要调整红利：
\begin{equation*}
...
\midrule
\multirow{3}{*}{\textbf{无红利}} 
& 1.需提前支付行权价，& 1.提前获得行权价格，\\
&\hspace{1em} 损失行权价格利息 &\hspace{1em} 可获得行前价格利息 \\
& 2.损失期权时间价值 & 2.失去期权时间价值 \\
\textbf{有红利} & 3.提前行权获得股票，获得红利 & 3.放弃红利 \\
\bottomrule
\end{tabular}
\end{table}
