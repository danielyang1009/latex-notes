...
\begin{equation*}
    \int^{S_T}_{S_0} \frac{S_T-K}{K^2} dK = \int^{S_T}_{S_0} \frac{(S_T-K)^+}{K^2} dK + \int^{S_0}_{S_T} \frac{(K-S_T)^+}{K^2} dK
\end{equation*}

由于$S_T$为随机变量在积分的上下限不方便处理，因此将其改写，加入积分为零的部分：
\begin{equation*}
    \int^{S_T}_{S_0} \frac{S_T-K}{K^2} dK = \int^{\infty}_{S_0} \frac{(S_T-K)^+}{K^2} dK + \int^{S_0}_{0} \frac{(K-S_T)^+}{K^2} dK
\end{equation*}

整理可得：
\begin{equation*}
    \Var = \frac{2}{T} \left[ \int^T_0 \frac{dS_t}{S_t} + \frac{S_T-S_0}{S_0} - \int^{\infty}_{S_0} \frac{(S_T-K)^+}{K^2} dK - \int^{S_0}_{0} \frac{(K-S_T)^+}{K^2} dK \right]
\end{equation*}

最后求风险中性下的期望期望得到最后的结果
