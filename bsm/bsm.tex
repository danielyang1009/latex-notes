...
可以看到期权合约对于分红的调整有如下几步，先根据ETF27日（除息日30日前一交易日）的收盘价及分红调整合约单位，其实质为缩放比例，为分红调整之后价格与分红之前价格比值的倒数。再根据新合约单位（缩放比例），等比例缩小行权价格与结算价格。合约单位比例放大与价格比例缩小相互抵消，使得合约名义价值与实际市值保持不变。
