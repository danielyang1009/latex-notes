...
\textbf{伊藤过程（Itô's Process）}，最一般化的随机过程，$a_t$和$b_t$为任意函数或随机过程。
\begin{equation*}
...
由于$\int^T_0 \sigma_t d\tilde{Z}_t$为随机波动项，因此取期望后为0。并且有$F=S_0 e^{rT}$，令$f(x)=\ln x$，利用下述两种方法展开$\ln\frac{S_T}{\kappa}$项则有：
\begin{align*}
    X_{var} &= \frac{2}{T} \tilde{E} \left[ - \ln\frac{S_T}{S_0 e^{rT}} \right] \\
    &= \frac{2}{T} \tilde{E} \left[ - \ln\frac{S_T}{F} \right] \\
    &= \frac{2}{T} \tilde{E} \left[ - \ln\frac{S_T}{\kappa} + \ln\frac{F}{\kappa} \right] \\
    &= \frac{2}{T} \tilde{E} \left[ - f'(\kappa) (S_T-\kappa) - \int^{\kappa}_{0} f''(K) (K-S_T)^+ dK - \int^{\infty}_{\kappa} f''(K) (S_T-K)^+ dK + \ln\frac{F}{\kappa} \right] \\
    &= \frac{2}{T} \tilde{E} \left[ - \frac{S_T-\kappa}{\kappa} + \ln\frac{F}{\kappa} + \int^{\kappa}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{\kappa} \frac{(S_T-K)^+}{K^2} dK \right]
\end{align*}

对于前两项，并使用泰勒级数分解$\ln \frac{F}{\kappa}$应有：
\begin{align*}
    \tilde{E} \left[ - \frac{S_T-\kappa}{\kappa} + \ln\frac{F}{\kappa} \right]
    &= -\frac{F-\kappa}{\kappa} + \ln \frac{F}{\kappa} \\
    &= -\frac{F-\kappa}{\kappa} + \left[ f(1) + f'(1)\left(\frac{F}{\kappa}-1\right) + \frac{1}{2}f''(1)\left(\frac{F}{\kappa}-1\right)^2 + \mathcal{O}\left( \frac{F}{\kappa} - 1 \right)^3 \right] \\
    &= -\frac{F-\kappa}{\kappa} + \left[ 0 + \left(\frac{F}{\kappa} - 1\right) - \frac{1}{2} \left(\frac{F}{\kappa} - 1\right)^2 + \mathcal{O}\left( \frac{F}{\kappa} - 1 \right)^3 \right] \\
    &= - \frac{1}{2} \left(\frac{F}{\kappa} - 1\right)^2 + \mathcal{O}\left( \frac{F}{\kappa} - 1 \right)^3
\end{align*}

对于后两项积分项，将其离散化则有：
\begin{align*}
    \tilde{E} \left[ \int^{\kappa}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{\kappa} \frac{(S_T-K)^+}{K^2} dK \right]
    &= \int^{\kappa}_{0} \frac{e^{rT}P(K)}{K^2} dK + \int^{\infty}_{\kappa} \frac{e^{rT}C(K)}{K^2} dK \\
    &\approx \sum_{K_i < \kappa} \frac{\Delta K_i}{K_i^2} e^{rT} P(K_i) + \sum_{K_i \geq \kappa} \frac{\Delta K_i}{K_i^2} e^{rT} C(K_i) \\
    &= \sum_{i} \frac{\Delta K_i}{K_i^2} e^{rT} Q(K_i)
\end{align*}

将上式中$\kappa$改写为$K_0$，最终得到方差计算如下：
\begin{equation*}
    \boxed{
        \sigma^2 = \sum_{i} \frac{\Delta K_i}{K_i^2} e^{rT} Q(K_i) - \frac{1}{2} \left(\frac{F}{K_0} - 1\right)^2 
    }
\end{equation*}

VIX为未来30天的波动率（年化）$\text{VIX}=\sigma_{30d} \times 100$。因此需要将两个期限的方差分别计算并加权平均：
\begin{equation*}
...
也可以将$\frac{dS_t}{S_t}$与$d\ln S_t$两式相减，进消去随机波动项$\sigma_t dZ_t$则有方差为：
\begin{align*}
    \text{Variance} &= \frac{1}{T} \int^T_0 \sigma^2 dt \\
    &= \frac{2}{T} \int^T_0 \left[ \frac{d S_t}{S_t} - d \ln S_t \right] \\
    &= \frac{2}{T} \left[ \int^T_0 \frac{d S_t}{S_t} - \int^T_0 d \ln S_t \right] \\
    &= \frac{2}{T} \left[ \int^T_0 \frac{d S_t}{S_t} - \ln \frac{S_T}{S_0} \right]
\end{align*}

在风险中性测度下对其求期望：
\begin{align*}
    \Var &= \frac{2}{T} \tilde{\E} \left[ \int^T_0 \frac{d S_t}{S_t} - \ln \frac{S_T}{S_0} \right] \\
...
其中对数项$\ln\frac{S_T}{S_0}$包含随机变量$S_T$，可用以下两种方式进行计算。

\subsubsection*{方法1：积分型余项的泰勒公式}
在Carr和Wu（2006）《A Tale of Two Indices》中使用了带积分余项（Integral form of the remainder）的泰勒公式进行求解（三种余项分别为：积分余项、Lagrange余项和Peano余项）。令$f(x) = \ln x$，$x=S_T$，$a=S_0$和$n=1$，则有：
\begin{equation*}
    f(S_T) = \frac{f(S_0)}{0!}(S_T-S_0)^0 + \frac{f'(S_0)}{1!}(S_T-S_0)^1 + \frac{1}{1!} \int^{S_T}_{S_0} f''(K) (S_T - K)^1 dK
\end{equation*}

整理后得到：
\begin{equation*}
    \ln S_T = \ln S_0 + \frac{S_T-S_0}{S_0} - \int^{S_T}_{S_0} \frac{S_T-K}{K^2} dK
\end{equation*}

由于不能确定$S_T$与$S_0$之间的大小，因此将最后一项积分分解。第一项为$S_T \geq S_0$的情形，而第二项为$S_T \leq S_0$的情形：
\begin{equation*}
    \int^{S_T}_{S_0} \frac{S_T-K}{K^2} dK = \int^{S_0}_{S_T} \frac{(K-S_T)^+}{K^2} dK + \int^{S_T}_{S_0} \frac{(S_T-K)^+}{K^2} dK
\end{equation*}

由于$S_T$为随机变量在积分的上下限不方便处理，因此将其改写，加入积分为零的部分：
\begin{equation*}
    \int^{S_T}_{S_0} \frac{S_T-K}{K^2} dK = \int^{S_0}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{S_0} \frac{(S_T-K)^+}{K^2} dK 
\end{equation*}

整理可得：
\begin{equation*}
    \ln S_T = \ln S_0 + \frac{S_T-S_0}{S_0} - \int^{S_0}_{0} \frac{(K-S_T)^+}{K^2} dK - \int^{\infty}_{S_0} \frac{(S_T-K)^+}{K^2} dK 
\end{equation*}
...
参考Carr和Madan（1998）在《Towards a Theory of Volatility Trading》中推导。假设$f(x)$为payoff，则根据狄拉克$\delta$函数的Sifting property改写函数为积分形式，并将积分分解为两部分（$\kappa$非负）。为方便后续计算，利用狄拉克$\delta$函数偶函数的性质将第一部分积分改写为$K-F$。
\begin{align*}
    f(F) &= \int^{\infty}_{0}f(K)\delta(F-K)dK \\
    &= \int^{\kappa}_{0}f(K)\delta(F-K)dK + \int^{\infty}_{\kappa}f(K)\delta(F-K)dK \\
    &= \int^{\kappa}_{0}f(K)\delta(K-F)dK + \int^{\infty}_{\kappa}f(K)\delta(F-K)dK
\end{align*}
...
\begin{align*}
    \int^{\kappa}_{0}f(K)\delta(K-F)dK 
    &= \bigg[ f(K) \mathbbm{1}(F<K) \bigg]^{\kappa}_{0} - \int^{\kappa}_{0} f'(K) \mathbbm{1}(F<K) dK \\ 
    &= f(\kappa) \mathbbm{1}(F<\kappa) - \cancel{f(0)\mathbbm{1}(F<0)} - \int^{\kappa}_{0} f'(K) \mathbbm{1}(F<K) dK \\
    &= f(\kappa)\mathbbm{1}(F<\kappa) - \int^{\kappa}_{0} f'(K) \mathbbm{1}(F<K) dK
\end{align*}
...
\begin{align*}
    \int^{\infty}_{\kappa} f(K)\delta(F-K)dK 
    &= \bigg[ - f(K) \mathbbm{1}(F \geq K) \bigg]^{\infty}_{\kappa} + \int^{\infty}_{\kappa} f'(K) \mathbbm{1}(F \geq K) dK \\
    &= - \cancel{f(\infty) \mathbbm{1}(F\geq \infty)} + f(\kappa) \mathbbm{1}(F\geq \kappa ) + \int^{\infty}_{\kappa} f'(K) \mathbbm{1}(F \geq K) dK \\
    &= f(\kappa) \mathbbm{1}(F\geq \kappa ) + \int^{\infty}_{\kappa} f'(K) \mathbbm{1}(F \geq K) dK
\end{align*}

将两部分积分代入原式整理可得：
\begin{align*}
    f(F) &=  f(\kappa)\mathbbm{1}(F<\kappa) + f(\kappa)\mathbbm{1}(F \geq \kappa)
    - \int^{\kappa}_{0} f'(K) \mathbbm{1}(F<K) dK 
    + \int^{\infty}_{\kappa} f'(K) \mathbbm{1}(F \geq K) dK
\end{align*}

再次使用分部积分法继续分解上式中的两项积分。对于指示函数的积分$\int^{\kappa}_{0}\mathbbm{1}(F<K) dK$，即计算高度为1的矩形面积。其宽度为$F$至积分上限$\kappa$，如果$F<\kappa$则有其积分（面积）为$\kappa-F$；如过$F>\kappa$，则有积分值为0：
\begin{equation*}
    \int^{\kappa}_{0}\mathbbm{1}(F<K) dK =
    \begin{cases}
        \kappa - F \; & F<\kappa \\
        0 \; & F>\kappa
    \end{cases}
...
\begin{align*}
    \int^{\kappa}_{0} f'(K) \mathbbm{1}(F<K) dK 
    &= \bigg[ f'(K)(K-F)^+ \bigg]^{\kappa}_{0} - \int^{\kappa}_{0} f''(K) (K-F)^+ dK \\
    &= f'(\kappa)(\kappa-F)^+ - \cancel{f'(0)(0-F)^+} - \int^{\kappa}_{0} f''(K) (K-F)^+ dK \\
    &= f'(\kappa)(\kappa - F)^+ - \int^{\kappa}_{0} f''(K) (K-F)^+ dK
\end{align*}

对于第二项积分，令$u=f'(K)$，$du=f''(K)dK$，同时需注意链式法则带来的符号改变$dv=\mathbbm{1}(F \geq K)dK$，$v=-(F-K)^+$：
\begin{align*}
    \int^{\infty}_{\kappa} f'(K) \mathbbm{1}(F\geq K) dK 
    &= \bigg[ -f'(K)(F-K)^+ \bigg]^{\infty}_{\kappa} + \int^{\infty}_{\kappa} f''(K) (F-K)^+ dK \\
    &= - \cancel{f'(\infty)(F - \infty)^+} + f'(\kappa)(F - \kappa)^+ + \int^{\kappa}_{0} f''(K) (F-K)^+ dK \\
    &= f'(\kappa)(F - \kappa)^+ + \int^{\infty}_{\kappa} f''(K) (F-K)^+ dK
\end{align*}

整理，最终结果为：
\begin{align*}
    f(F) &= f(\kappa)\mathbbm{1}(F<\kappa) + f(\kappa)\mathbbm{1}(F \geq \kappa) \\
    &\quad - \left[ f'(\kappa)(\kappa - F)^+ - \int^{\kappa}_{0} f''(K) (K-F)^+ dK \right]
    + \left[ f'(\kappa)(F - \kappa)^+ + \int^{\infty}_{\kappa} f''(K) (F-K)^+ dK \right] \\
    & = f(\kappa) + f'(\kappa) \left[ (F-\kappa)^+ - (\kappa - F)^+ \right] + \int^{\kappa}_{0} f''(K) (K-F)^+ dK + \int^{\kappa}_{0} f''(K) (F-K)^+ dK \\
    & = f(\kappa) + f'(\kappa) (F-\kappa) + \int^{\kappa}_{0} f''(K) (K-F)^+ dK + \int^{\infty}_{\kappa} f''(K) (F-K)^+ dK
\end{align*}
...
对期权进行定价往往采用风险中性定价法，在风险中性的世界里，所有资产的预期收益率都等于无风险利率。同样对于未来一段时间内的波动率平均值的估计也可以认为是一种广义的“定价”，因此从t时刻到到期日T这段时间内的广义波动率被定义为风险中性的世界里从0时刻到T时刻期间的方差序列的平均值的平方根，其方差可以表示为：
\begin{align*}
    \sigma^2_{T} &= \frac{1}{T} \tilde{\E}\left[ \int^T_0 \sigma^2_s ds \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \int^T_0 \frac{d S_t}{S_t} - \ln \frac{S_T}{S_0} \right] \\
    &= \frac{2}{T} \tilde{\E} \left[ \int^T_0 \frac{d S_t}{S_t} - \frac{S_T-S_0}{S_0} + \int^{S_0}_{0} \frac{(K-S_T)^+}{K^2} dK + \int^{\infty}_{S_0} \frac{(S_T-K)^+}{K^2} dK \right] \\
    &= \frac{2}{T} \\
\end{align*}
