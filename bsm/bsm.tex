...
虽然一般将无模型波动率认为是“BS隐含波动率的一种加权平均”更方便理解，但实际无模型波动率与BS模型完全没有关系。准确的说无模型并不是完全没有模型，只是这样的波动率估计只需要假定股票价格的随机过程为$dS_t/S_t =\mu_t dt + \sigma_t dZ_t$，其中$\mu_t$与$\sigma_t$均为时变，即微观上类几何布朗运动，而不需要其他更严格的假定。VIX并非某一合约的隐含波动率，而是未来一段时间内波动率（Total variance）平方的期望：
\begin{equation*}
    \Var^2 = \frac{1}{T} \int^T_0 \sigma^2 dt
\end{equation*}
...
使用Ito公式可得，对数收益率为：
\begin{equation*}
    d \ln S_t = \left(\mu - \frac{\sigma^2}{2} \right) dt + \sigma dZ_t
\end{equation*}

两式想减可得：
\begin{equation*}
    \frac{dS_t}{S_t} - d\ln S_t = \frac{\sigma^2}{2} dt
\end{equation*}

则有方差为：
\begin{align*}
    \Var &= \frac{1}{T} \int^T_0 \sigma^2 dt \\
...
此时第一项积分较容易离散并计算，第二项对数项$\ln\frac{S_T}{S_0}$包含随机变量$S_T$，有以下两种方式进行计算。

\subsubsection*{方法1：积分型余项的泰勒公式}
在Carr和Wu（2006）中使用了带积分余项（Integral form of the remainder）的泰勒公式进行求解（三种余项分别为：积分余项、Lagrange余项和Peano余项）。令$f(x) = \ln x$，$x=S_T$，$a=S_0$和$n=1$，则有：
\begin{equation*}
...
\begin{equation*}
    H'(F-K) = \frac{\partial H(F-K)}{\partial F-K} \frac{\partial F-K}{\partial K} = \delta(F-K)\cdot(-1)
\end{equation*}
