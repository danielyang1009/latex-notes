...
\usepackage[a4paper,left=4em,right=4em,top=4em,bottom=4em]{geometry}
\usepackage{amsmath,amsfonts,amsthm,bbm}
\usepackage{booktabs,float,multirow}
\usepackage{enumitem}
\setitemize{leftmargin=2em,itemsep=0em}
\usepackage{cancel}
\usepackage{graphicx}
\usepackage{footmisc}
\usepackage{hyperref}
...
对于一个连续且在$[0,T]$中处处可微的函数$f(t)$，$t_{i^*} \in [t_{i},t_{i+1}]$，利用微分中值定理，可以看到随着时间的越来越细的划分，$\left\Vert\Pi \right\Vert$趋近于零。则可得到处处可微的连续函数，在区间$[0,T]$的二次变分为0。
\begin{align*}
    \sum_{i=0}^{N-1} \left[ f(t_{i+1}) -f(t_{i}) \right]^2 
    &= \sum_{i=0}^{N-1} f'(t_{i^*})^2 (t_{i+1} - t_{i})^2 \\
    &= \left\Vert \Pi \right\Vert \sum_{i=0}^{N-1} f'(t_{i^*})^2 (t_{i+1} - t_{i})
\end{align*}

于是有：
\begin{align*}
    [f,f](T) &= \lim\limits_{||\Pi|| \rightarrow 0} \left\Vert \Pi \right\Vert \left[ \sum_{i=0}^{N-1} f'(t_{i^*})^2 (t_{i+1} - t_{i}) \right] \\
    &= \lim\limits_{||\Pi|| \rightarrow 0} \left\Vert \Pi \right\Vert \cdot \lim\limits_{||\Pi|| \rightarrow 0} \sum_{i=0}^{N-1} f'(t_{i^*})^2 (t_{i+1} - t_{i}) \\
    &= \lim\limits_{||\Pi|| \rightarrow 0} \left\Vert \Pi \right\Vert \cdot \int^T_0 f'(t_{i^*})^2 dt = 0
\end{align*}
