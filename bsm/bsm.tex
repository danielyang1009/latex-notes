...
同样由于持有远期合约并不实际持有现货，若在远期合约到期期间，标的资产会产生与现货价格成一定比率为$q$的红利，应剔除：
\begin{equation*}
    f(t,S_t) = e^{-r(T-t)} [S_t e^{(r-q)(T-t)} - K] = 0
\end{equation*}
...
看跌-看涨期权平价关系（Put-call parity），指具有相同的行权价与到期日的欧式看涨期权与欧式看跌期权，其价格之间存在的关系。若平价关系不成立，则意味着存在套利的空间。现实市场中存在交易成本，因此买卖权平价关系不完全成立。然而流动性较好的市场中，可以近似认为买卖权平价关系成立。
...
TODO  ETF无红利保护，股指期货有红利保护，两者进行PCP需要调整红利
ETF标的现货 与 期权，不需要调整？
股指期货 F 与 期权 不需要调整？

\begin{align*}
    C_{t,T} &= P_{t,T} +  S_t - K e^{-r(T-t)} \\
    C_{t,T} &= P_{t,T} + \left( F_{t,T} e^{-r(T-t)} + I_t \right) - K e^{-r(T-t)}
\end{align*}

隐含期货价格为：
\begin{equation*}
    F_{t,T} = \left( C_{t,T} - P_{t,T} \right) e^{r(T-t)} + K - Ie^{r(T-t)}
\end{equation*}
