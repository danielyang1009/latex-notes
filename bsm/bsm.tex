...
\begin{equation*}
    \text{VIX}^2 = \frac{1}{T} \int^T_0 \sigma^2 dt
\end{equation*}
...
\begin{equation*}
    ...
    = \frac{2}{T} \int^T_0 \left( \frac{d S_t}{S_t} - \ln \frac{S_T}{S_0} \right) 
\end{equation*}

在Carr和Wu（2006）中使用了带积分余项（Integral form of the remainder）的泰勒公式（三种余项分别为：积分余项、Lagrange余项和Peano余项）：
\begin{equation*}
    f(x) = \sum^n_{k=0}\frac{f^{(k)}(a)}{k!}(x-a)^k + 
    \frac{1}{n!} \int^x_a f^{(n+1)}(t)(x-t)^n dt
\end{equation*}

带入$f(x) = \ln x$，$x=S_T$，$a=S_0$和$n=1$，则有：
\begin{align*}
    f(S_T) &= \frac{f(S_0)}{0!}(S_T-S_0)^0 + \frac{f'(S_0)}{1!}(S_T-S_0)^1 + \frac{1}{1!} \int^{S_T}_{S_0} f''(K) (S_T - K)^1 dK  \\
    \ln S_T &= \ln S_0 + \frac{S_T-S_0}{S_0} - \int^{S_T}_{S_0} \frac{S_T-K}{K^t} dK
\end{align*}
...
对于狄拉克函数（Dirac delta function）有，对其平移$\delta(x-3)$，即在$x=3$为无穷：
\begin{equation*}
    \delta(x) =
    \begin{cases}
        +\infty \qquad  &x=0 \\
...
此时可以发现$d_\tau$的积分，$\int^{+\infty}_{-\infty} d_\tau(x) dx = 1$。当$\tau$不断变小的时候$\lim_{\tau \rightarrow + \infty} d_{\tau}(t) = \delta(x)$，那么此时对于狄拉克函数，其积分应有：
\begin{equation*}
