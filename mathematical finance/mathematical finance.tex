...
可以将Delta理解为期权价格与标的价格的贴合程度，以看涨期权为例，当为深度实值时，贴合程度最好，标的价格变化1单位，期权价格变化1单位。即虚值贴合差，对股价变动不敏感，而实值贴合好，对股价变动敏感。如下图所示，随着到期时间不断减少，期权价格曲线将越来越贴合期权内在价值。
...
做多期权，有正的Gamma，能使得价格变化对投资者有利。即当价格上涨的时候，对投资者有利，变化越来越快，盈利越来越多。相反，当价格下跌时，对投资者不利，变化越来越慢，损失越来越小。而作为做市商等，卖期权，或做空期权，则有负的Gamma。此时与期权投资者相反，价格上涨时，盈利减慢，而价格下跌时，损失增大。正Gamma为凸函数（Convex，下凸），负Gamma为凹函数（Concave，上凸），另外可以看到在平值点的Gamma最大。
...
Vega体现的为期权价格对波动率的敏感程度，即波动率变化$1\%$对期权价格的影响。对于期权的多头，其Vega为正，意味着隐含波动率的正向变化对其有利。
...
下图期权的时间价值曲线中，随着期限的减少，期权的时间价值在加速衰减。因此对于期权的多方，其时间价值在不断衰减，有Theta为负。相反，空方则Theta为正。
