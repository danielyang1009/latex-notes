...
在金融研究中，在计算收益率的是哦户，一般使用对数收益率。其对比百分比收益率的优势在于，对于单一资产的收益率，对数收益率时序可加。如一个资产在在第一年上涨50\%，而在第二年下跌50\%，其算术平均收益率为0（即时序上不可加），但显然该资产年年的投资亏损的初始金额的25\%。同样的例子，若使用对数收益率计算，则该资产两年的对数收益率分别为$\ln(1.5) = 40.5\%$与$\ln(0.5) = -69.3\%$，利用对数收益率时序可加的特点，其两年算术平均为$-28.77\%$，转换为百分比收益率为$e^{-28.77\%} - 1 = -25\%$，为实际真实的收益率。可以看到上述对数收益率与百分比收益率的计算是等价的，但是能大大降低运算难度。

由于对数收益率时序上可加的特点（线性），即只需要对单期的收益率进行加总即可获得多期的收益率：
\begin{align*}
    \ln\frac{X_T}{X_0} &= \ln \left[ \frac{X_1}{X_0} \times \frac{X_2}{X_1} \times \cdots \times \frac{X_T}{X_{T-1}} \right] \\
    &= \ln\frac{X_1}{X_0} + \ln\frac{X_2}{X_1} + \cdots + \ln\frac{X_T}{X_{T-1}}
\end{align*}

并且利用该特点，可以得到“中心极限定理”与“大数定理”。若不同期之间的对数收益率相互独立，那么T期的收益率相加，相当于T个独立随机变量相加，由中心极限定理可知，其和的分布将逼近正态分布。并且由“大数定理”可知，随着T的增大，单期收益率的均值，将收敛于期望：
\begin{equation*}
    \frac{1}{T} \left[ \ln\frac{X_1}{X_0} + \ln\frac{X_2}{X_1} + \cdots + \ln\frac{X_T}{X_{T-1}} \right] \rightarrow \E\left[ \ln\frac{X_T}{X_0} \right] \quad as \quad T \rightarrow \infty
\end{equation*}

对数压缩了数据的尺度，使得数据在较小时更敏感。对比上证指数与上证指数取对数后，在取对数之前可以看到05-07年大牛市从998到6124取得了600\%的收益。而在取了对数之后，可以清晰看到，90年代末期的牛市从95到1558点，得到1500\%的收益率。即随着指数数值越来越大，绝对数值的变化越来越大，使得难以观察其相对数值的变化。以2为底取对数为例，$\log_2 2 =1$，$\log_2 4 =2$，$\log_2 8 =3$，可以看到取对数后每单位的变化，对应着相同百分比的变化。以2为底，即对应着2倍的百分比变化$2^{x+1}/2^x$。因此取对数的方法，是百分比变化为纵轴，而正常画图的方法，则是以绝对数值纵轴。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/sse-normal-scale.png}
    \caption{上证指数}
    \label{fig:sse-normal-scale}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/sse-log-scale.png}
    \caption{上证指数取对数}
    \label{fig:sse-log-scale}
\end{figure}

而对于不同资产在截面上的收益率，则应使用百分比收益率，具有截面可加性。以两个资产为例，投资资金为$m_1$与$m_2$，期末百分比收益率为$r_1$与$r_2$，那么此时投资组合的收益率有：
\begin{align*}
    r_p &= \frac{m_1(1+r_1) + m_2(1+r_2)}{m_1+m_2} - 1 \\
    &= \frac{m_1}{m_1 + m_2} r_1 + \frac{m_2}{m_1 + m_2} r_2
\end{align*}

可以看到此时投资组合在截面上的收益率即为各个资产百分比收益率的投资金额加权平均。

\begin{note}
    在Excel中，计算算术平均函数为\code{AVERAGE()}，而计算几何平均函数为\code{GEOMEAN()}。
\end{note}

\subsection{对数收益率推导}

对数收益率（Log return），或称连续复合收益率（Continuously compounded return），以复合利率为例。假设名义利率（Nominal interest rate）为$r$，进行再投资，且一年计息$m$次，则有实际利率（Effective interest rate，EIR）为：
\begin{equation*}
    \begin{aligned}
...
如上公式所示：
\begin{equation*}
    \text{Delta} = \dfrac{\partial V}{\partial S}
\end{equation*}

Delta描述了标的价格变动，对期权价格的影响，当$\text{Delta}=0.5$时，假设标的资产变动1元，那么期权价格将变动$1\times 0.5 = 0.5$元。显然应有$\abs{\text{Delta}} \in (0,1)$。如下图所示看涨期权的Delta值在$(0,1)$之间，看跌期权的Delta值在$(-1,0)$之间。对于BSM而言，看涨期权Delta与看跌期权Delta有如下关系$\text{Delta}_C - \text{Delta}_P = 1$。期权价格与标的资产价格的关系如下：
\begin{figure}[H]
    \centering
...
\subsubsection*{Dollar Delta}

Delta exposure或称为现Dollar Delta（现金Delta）或记为$\$\Delta$，当投资期权时，希望知道当前的期权投资，\uline{相当于投资了多少头寸的标的资产（金额计）}（cash equivalent exposure of the underlying）。由$\Delta$的定义可知，其为对于价格敏感程度的指标，同时也可以通过$\Delta$，将其换算为数量之比。假设当前看涨期权$\Delta=0.5$，标的资产价格为100元，那么标的资产每变动1元，期权价格变动0.5元。那么此时，对于持有的每份期权，相当于持有0.5份的标的资产，乘以标的资产的价格，即得到$\$\Delta = \Delta \times S = 0.5 \times 100 = 50$。即当前对于这一份期权，相当于投资50元于标的资产。

使用Dollar Delta的好处是，方便不同标的资产价格的期权之间进行比较。假设股票A价格为100元，股票B价格为50元，此时两者期权的Delta都为0.5，对于一份期权而言，对于股票A期权，相当于投资了50元于股票A，而对于股票B，相当于投资了25元于股票B。在组合中，若直接将Delta相加，Delta为1。若一个看涨一个看跌，两者相加为0，并没有意义。

备注：以50ETF期权为例，合约单位为10000份，当$\Delta=0.5$时，那么一手期权即对应了5000份个股。

\subsubsection{Gamma}

同时Gamma如图\ref{fig:delta-stock}中所示，为Delta与标的资产价格关系的一阶导（资产为线性，$\Gamma=0$）：
\begin{equation*}
    \Gamma = \frac{\partial \Delta}{\partial S} = \frac{\partial^2 V}{\partial S^2}
\end{equation*}

若以期权价格为纵轴，标的资产价格为横轴，得到期权收益，如图\ref{fig:call-vs-stock}。那么Delta为该曲线切线的斜率，为一阶导，衡量期权价格对标的资产变动的敏感程度。Gamma为该曲线的二阶导，为Delta的变化速度，或曲线弯曲程度或加速度，衡量Delta对标的资产价格变动的敏感程度，或标的价格没变化1元，引起的Delta值的变动，假设标的资产价格为200，Delta为0.3，Gamma为0.02，那么当标的资产价格上升1元至201时，那么Delta上升至0.32。
...
\subsubsection*{Dollar Gamma}

Gamma exposure或称为Dollar Gamma（现金Gamma）或记为$\$\Gamma$。从下文对BSM模型Delta推导中可知，看涨看跌期权的Delta值分别为：$\Delta_C=N(d_1)$，$\Delta_P = N(d_1)-1$，Detla值并不与标的资产价格（水平）高低相关。而对于Gamma而言，对于看涨看跌期权其Gamma相同，即Detla对标的资产价格S球导：$\Gamma = N'(d_1)/(S \sigma \sqrt{\tau})$，可以发现Gamma与标的资产价格水平的倒数有关。因此，对于其他条件相同的两支期权，若A股价为B股票的2倍，那么A的Gamma为B的$\frac{1}{2}$，这样显然无法直接进行对比。

由下文希腊值的分解可知，若期权价值为$V$，对标的资产价格$S$与时间$t$进行泰勒展开，并忽略高与$\Delta t$的项（保留前三项），若忽略Theta的影响可得：
\begin{align*}
    dV &= \frac{dV}{dS} dS + \frac{dV}{dt} dt + \frac{1}{2}\frac{d^2 V}{d S^2}(dS)^2 \\
    &\approx \Delta dS + \frac{1}{2}\Gamma (dS)^2
\end{align*}

由上对Dollar Delta的分析可知：
\begin{equation*}
    \$\Delta = \Delta \times S = \frac{d V}{d S} \times S = \frac{dV}{(dS/S)}
\end{equation*}

通过变换，那么可以得到收益率的一阶展开，其意义也方便理解，即期权合约价值的变化，等于相当于投资了多少金额的标的资产，乘以标的资产价格的变动：
\begin{equation*}
    dV = (\$\Delta) \frac{dS}{S}
\end{equation*}

此时可以看到同时$\$\Delta$还衡量了，\uline{期权合约价值对标的资产每变动1\%的敏感程度}。同样以标的资产价格$S=100$，期权$\Delta=0.5$为例：若持有一份期权合约，此时$\$\Delta=50$，相当于买了半份标的资产（金额）；同时当标的资产价格变化每1\%，即标的资产变动1元，期权价格变化0.5元，此时$\$\Delta = 0.5/0.01 = 50$。

若将如上概念自然扩展至二阶：
\begin{equation*}
    dV \approx (\$\Delta) \frac{dS}{S} + \frac{1}{2} (\$\Gamma) \left(\frac{dS}{S}\right)^2
\end{equation*}

此时定义Dollar Gamma（$\$\Gamma$）为：
\begin{align*}
    \$\Gamma &= \Gamma \times S^2 = \frac{d \Delta}{d S} \times S^2 \\
    &= \frac{d \Delta \times S}{(dS/S)} = \frac{d \Delta \times S}{(dS/S)} \\
    &= \frac{d \$\Delta}{dS/S}
\end{align*}

可以看到此时Dollar Gamma衡量了，\uline{Dollar Delta对标的资产价格每1\%变动的敏感程度}。
...
【待整理】 在Bakshi and Kapadia 2003中，在一段时间内的使用看涨看跌期权进行Delta对冲（\textbf{注意：}由几何布朗运动与伊藤过程推到而来，因此看涨看跌期权形式相同，买入看涨看跌期权，并卖出股票，扣除净投资金额所应获得无风险收益，即计算超额收益）。
\begin{equation*}
...
假设持有时间为t到t+1，以看涨期权为例：具体操作为在t时刻时，买入1份看涨期权（消耗现金$-C_{t}$），卖出$\Delta_C$份股票（获得现金$+ \Delta_C S_{t}$）。而在t+1时刻进行平仓，即卖出1份看涨看跌期权（获得现金$C_{t+1}$），买入$\Delta_C$份股票（消耗现金$- \Delta_C S_{t+1}$）。因此整体投资组合的收益应为：
\begin{equation*}
    ( C_{t+1} -\Delta_C S_{t+1} ) + ( -C_{t} + \Delta_C S_{t} ) = (C_{t+1} - C_{t}) - \Delta_C (S_{t+1} - S_{t})
\end{equation*}

对于看跌期权，同理与看涨期权公式相同，但此时需注意看跌期权$\Delta_P<0$为负：在t时刻，买入1份看跌期权（消耗现金$-P_{t}$），“卖出”$\Delta_P$份股票（注意看跌期权$\Delta$负，实则买入$\abs{\Delta_P}$份股票，“获得”（消耗）现金$+ \Delta_P S_{t}$）。在t+1时刻进行平仓，即卖出1份看跌期权（获得现金$P_{t+1}$），“买入”$\Delta$份股票（实则为卖出$\abs{\Delta_p}$份股票，“消耗”（获得）现金$- \Delta_p S_{t+1}$）。因此整体投资组合的收益应为：
\begin{equation*}
    ( P_{t+1} - \Delta_P S_{t+1} ) + ( -P_{t} + \Delta_P S_{t} ) = (P_{t+1} - P_{t}) - \Delta_P (S_{t+1} - S_{t})
\end{equation*}

vega-neutral delta-hedged P\&L
买call卖put（相同K），但还有delta和gamma暴露，没有vega与theta暴露
...
VIX为年化未来30天（日历日或约22天交易日）的波动率，将两个不同期限的方差进行线性插值即可得到：
\begin{equation*}
...
定价思路为由于在签订远期合约时，双方签订在未来$T$时刻的交割价格（Delivery price），应基于双方公平的原则（Zero net market value at entry），使得远期合约签订之时其价值（Forward value）为零或$f(0,S_0)=0$。因此得到远期价格（Forward price）$F_{t,T}$，即在t时刻，使得远期合约价值为零的合理交割价格（可以理解为在t时刻不断的签订新的远期合约）。但在该合约签订之后，随着标的资产价格的变化，该合约的价值（$f$）将不断发生变化。

在无套利的框架下，并且远期价值和远期价格都只取决于\uline{当前的现货价格}与\uline{持有成本}，不额外反应对未来现货的预期。
\begin{itemize}
    \item $F_{t,T}$：在t时刻时，到期时间为T的理论远期价格（标的物价格）
    \item $f(t,S_t)$：远期合约在t时刻的价值（合约价格）
    \item $S_t$：远期（期货）标的资产在t时刻的价格
    \item $K$：远期合约中的交割价格（Delivery price）
\end{itemize}

远期与期货的到期交割，保证了到期时远期或期货的价格与现货相同。另外需要注意，期货为标准化的远期合约，期货有逐日盯市制度（Mark-to-market），在每日现货发生变化，使得期货价格发生变化，根据合约的结算价格，计算各个投资者的盈亏，以此调整保证金账户。那么这样就相当于新签订一个到期时间不变的远期合约，即若不考虑期货合约日内的变化，期货合约的价值为零（每日结算将期货合约归零）。因此期货价格本质是交割价格或远期价格，而不是期货价值，由于期货价值日内很小，且当日结算后归0。对于现金交割，由于盯市制度，因此交割实际反应的即当日的盈亏。同时，由于期货的盯市制度，使得每日的盈亏为“实盈实亏”，而远期则为“浮动盈浮亏”。多头若因市场下跌，而被强制平仓，那么其便由多头转为空头。反之，空头被强制平仓，则转换为多头，都将进一步加剧市场的波动。

并且与期权合约不同，期权买卖双方权利义务不对等。而期货买卖双方都有对等的权利与义务，期货买卖双方都需要支付保证金。对于期货的多头与空头，多头是持有资金购入商品，而空头是持有商品卖出获得资金。由于资金相较于商品更容易筹得，因此多头往往占据优势地位，容易进行逼空，即空头交不出那么多的商品。
...
在签订之时基于双方公平的原则,应有合约在签订$t=0$之时，远期合约的价值应为0。因此可以确定合理的交割价格$K$，即为合理的远期价格$F_{0,T}$：
\begin{equation*}
    f(0,S_0) = S_0 - e^{-rT}K = 0
\end{equation*}

此时合理的交割价格$K = F_{0,T} = S_0 e^{rT}$。\textbf{远期价格}的定义为在t时刻，使得远期合约价值为0的交割价格。因此在t时刻，远期价格与现货价格之间的关系应为：
\begin{equation*}
    \boxed{
        F_{t,T} = S_t e^{r(T-t)}
    }
...
假设投资者A购买了一份远期合约（多头），在$T$时刻到期时，履约时需要支付现金$K$，用于购买标的物其价格为$S_T$。因此在$t$时刻，此投资者持有的投资组合应为$f_t + K e^{-r(T-t)}$。假设投资者B，在$t$时刻购买该远期合约的标的物，价格为$S_t$，并持有到期。两位投资者到期时都将持有一单位的标的资产，届时其价格为$S_T$，显然两个投资者在$t$时刻所拥有的投资组合价格应该相等，否则可进行套利，因此应有：
\begin{equation*}
        f(t,S_t) + K e^{-r(T-t)} = S_t 
\end{equation*}

由上可知，基于签订双方公平的原则，签订的远期价格$F_{t,T}$，为使得远期合约为零的交割价格K，因此远期价格为$F_{t,T} = S_t e^{r(T-t)}$。对于金融期货而言，若无红利，可以发现远期价格$F_{t,T}$与现货价格$S_t$之间仅相差货币时间价值，这部分货币的时间价值可理解为持有成本。购买远期不占用资金，无资金成本（假设保证金也会获得相应占用资金无风险利息）。相反购买现货则需要占用资金，应支付相应的无风险利息作为其持有成本。

\subsection{红利资产调整}

\subsubsection*{已知红利}

远期合约期限之内，标的资产产生红利数额为$D$，其红利现值为$I_t$，由于股票进行分红后需要对股票进行除息处理，股票（现货）价格回落，并且由于持有远期合约并持有现货，因此不能获得红利，因此也应下调远期价格。则有$S_t-I_t$为剔除红利的影响后，使其变为无红利资产。此时远期价格$F_{t,T}$为使得远期合约价值为零的交割价格K：
\begin{equation*}
    f(t,S_t) + K e^{-r(T-t)} = \left( S_t - I_t \right) = 0
\end{equation*}
...
如上所述，若标的资产支付与现货价格成一定比率为$q$的红利（红利率），由于将使得股指回落，且持有远期不能获得红利，因此同样应剔除其影响。如对于股指期货而言，其成分会不断的进行分红，相当于是红利率，因此也应进行红利调整。由上述复制定价可知：
\begin{equation*}
    f(t,S_t) + K e^{-r(T-t)} = S_t e^{-q(T-t)} = 0
\end{equation*}
...
对于商品期货而言，基差（Basis）指现货与远期价格之差（现货价格Spot price有时又被称为Cash price），描述了期限结构上两者的关系。通常基差指期限短减去期限长，而升贴水指对于期货而言，期货价格（期限长）大于现货价格（期限短）为升水，反之亦然。对于基差有：
\begin{equation*}
...
那么对于商品期货而言：
\begin{itemize}
    \item 负基差：期货价格高于现货价格，净持仓成本为正，期货升水（Contango，支付交易延期费）。此时的市场被称之为正向市场或正常市场（Normal market），即期限较短的合约价格（Near-maturity contract）（或现货）低于期限较远的合约价格（Far-maturity contract），期限结构斜率为正。由于大多数\uline{商品期货储藏成本较大，大于便利收益，使得期货升水}，因此称为正向市场。
    \item 正基差：期货价格低于现货价格，净持仓成本为负，期货贴水（Backwardation）。此时的市场称为逆向市场（Inverted market），即期限较短合约价格（或现货）高于期限较远的合约，期限结构斜率为负。
    \item Normal Contango与Normal backwardation指当前期货价格与预期未来现货价格$\E_t[S_T]$的升贴水关系，为风险溢酬
\end{itemize}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption{商品期货市场$F_{t,T}$与$S_t$}
\end{figure}

\subsubsection*{中国股指期货}

需要格外注意的是，与商品期货相反，\uline{在金融期货中，一般定义基差为期货价格减去现货价格}，即：
\begin{equation*}
    \text{基差（金融期货）} = \text{期货价格} - \text{现货价格}
\end{equation*}

因此，当金融期货贴水，即期货价格小于现货价格，称有负基差。而金融期货升水，或期货价格大于现货价格，称有正基差。在中国市场中，在股指期货中一般存在期货贴水的现象，即现货价格$S_t$大于期货价格$F_t$（期货贴水），现货价格被高估。
...
\begin{itemize}
    ...
    \item 上证50、沪深300等标的融券较为容易，但中证500、中证1000等标的融券较为困难，还有一部分原因中证500等现货估值较高，因此中证500、1000贴水较为严重
...
如上部分讨论，对于无红利金融期货与现货，由于持有远期不占用资金，而持有现货需占用资金，即利息成本，因此有关系$F_{t,T} = S_t e^{r(T-t)}$。并且对于有红利资产，也可以调整为无红利资产进行计算。而对于商品期货等则需要额外考虑储藏成本与便利收益。

在上述复制定价的例子中，根据一价定律（Law of one price）与无套利条件，持有远期合约$f_{t,T}$（与交割价格现值的资金）与持有现货持有到期，两者到期都将持有一单位现货$S_T$。由于持有远期合约，只在规定到期日交割标的资产，并不持有实际现货，因此无法获得持有现货的收益，因此持有现货获得的收益应该从中扣除。同样，持有现货到合约到期日的成本需计算在远期价格内。因此在t时刻，任意远期（期货）价格$F_t$与现货价格$S_t$有如下无套利关系：
\begin{align*}
    F_{t,T} &= S_t + \textbf{净持有成本（Net cost of carry）} \\
    &= S_t + \text{合约期内持有成本（Carry cost）} - \text{合约期内持有收益（Carry return）} \\
    &= S_t + \text{利息成本} + \text{储藏成本（Storage cost）} - \text{红利收益} - \text{便利收益（Convenience yield）}
\end{align*}

\subsubsection*{不考虑便利收益}

利息成本无可避免，为资金成本成本，若持有成本为$c$，那么应有$F_{t,T} = S_t e^{(r+c)(T-t)}$。由两者的关系可以看到，由于利息成本与仓储成本等持有成本都较为确定，\uline{若不考虑便利收益（如金融期货）}，那么两者的差异并不反应投资者对未来的预期。期货与现货的定价信息，反应的都是现货的定价信息，两者之差反应的是持有成本，基于无套利条件得到，期货并没有额外的信息含量，并不包含预期信息，与未来价格关系很小。

期货交易为T+0，期货为保证金交易，占用资金少，期货交易更加活跃，对信息更敏感。因此当有信息冲击时，信息同时作用于现货与期货市场，价格变化首先体现在期货上，期货价格领先现货价格变化，此并非期货价格反应预期（现实测度，期货价格是风险中性下预期）。仅由于期货交易成本更低，交易速度更快，对信息的反应速度更快，领先于现货（在时间序列上的领先滞后关系，而非因果逻辑，可通过格兰杰因果、VAR、VECM检验），为期货的价格发现功能（Price discovery），\uline{为当前的期货价格发现当前的现货价格}。

即便经济因素最终发生在未来，但影响发生在当前，因此当市场有效或部分有效的前提下，经济因素作用应马上反应到当前资产价格中，期货价格变化能够实时高效传递现货价格变化的信息，发挥传递功能，不意味着期货价格可以预测未来价格。因此思考的逻辑是不是期货能“预测”现货，而是期货价格变化中，有那些是现货市场无法表达的“不能说出的秘密”。

对于商品而言，许多商品现货交易并不活跃，又如农产品受到种植周期的影响。以2018年苹果期货的大幅度上涨为例，苹果产地主要在陕西、甘肃与山西，当年清明苹果花期发生了大面积霜冻，将导致未来苹果减产，引发期货价格上涨，传递了当前苹果市场供求关系的信息，发出了预警信息。而若没有期货市场，可能需要等到当年八九月苹果上市，才能发现苹果大规模减产，而使得苹果相关的企业受到较大的冲击。

\subsubsection*{不完美市场}

上述期货价格与现货价格的关系建立在完美市场中，但要注意在现实世界中，可能存在种种约束，使得无套利条件并非等价关系，而是存在一个区间。当考虑交易成本时，若$F_{t,T}>S_t e^{r(T-t)}$，那么应该做空期货，做多现货，而交易成本使得购买现货成本上升，上限为$S(1+Y)e^{r(T-t)}$；若$F_{t,T}<S_t e^{r(T-t)}$，那么应该做多期货，做空现货，此时交易成本使得做空现货的收益降低，因此下限为$S(1-Y)e^{r(T-t)}$。当市场越有效，交易成本越低，无套利的区间也应更窄。此时的无套利区间应有：
\begin{equation*}
    \left[ S(1-Y)e^{r(T-t)}, S(1+Y)e^{r(T-t)} \right]
\end{equation*}

当考虑不同的借贷成本时，同样如上述讨论，若期货价格高，做空期货，借入资金买入现货，此时资金成本为$r_b$；相反，若期货价格低，那么此时应做多期货，做空现货并获得资金，可以进行出借，出借利率为$r_l$，此时的无套利区间为（一般借入利率更贵，借出利率更便宜，即$r_l<r_b$）。若借出利率更高，借入利率更低，即投资收益大于融资成本，那么可以直接进行投资（套利），将使得借入利率不断升高，投资收益不断降低。
\begin{equation*}
    \left[ S e^{r_l(T-t)}, S e^{r_b(T-t)} \right]
\end{equation*}

若考虑卖空限制，即在卖出现货时需要支付额外的成本，使得卖出现货获得资金减少。那么应有：
\begin{equation*}
    \left[ S(1-X) e^{r(T-t)}, S e^{r(T-t)} \right]
\end{equation*}

而此时若不允许现货卖空，那么即卖空现货成本趋近于无穷大，成本趋近于100\%，那么下限趋近于零。对于金融资产而言，上述约束条件对其影响较强，因此如股指期货与股票指数联动紧密，一般偏离理论价值不会太大。但对于商品期货而言，其买卖成本较大，且做空现货很难，因此难以满足上述无套利约束。

\subsubsection*{考虑便利收益}

对于商品期货，便利收益指实际持有持有现货带来的收益，如工厂用于生产的需求，即对现货的需求程度\uline{与预期有关}，令其为$y$，现货价格与便利收益往往正相关。当对现货需求越大，现货越贵，此时便利收益越高。便利收益为随机变量，且取决于短期内的供求关系。作为原油而言，当国际形势紧张，此时便利收益远大于储藏成本，因此此时现货价格将大于期货价格。即$F_{t,T} = S_t e^{(r+c-y)(T-t)}$，此时$y$大大升高，使得期货价格小于现货价格，期货贴水。相反，当疫情发生，全球工厂停工，需求大大降低，因此需求过剩，或供给充分。因此原油带来的便利收益$y$很低，储藏成本大于便利收益，此时原油期货价格大于现货价格，期货升水。

在负油价事件中，出现负油价的是马上要交割的期货，其价格实质为现货价格，而其他合约的原油期货都为正。交割期货时，多头无法找到储油设施存入原油，使得短期内储藏成本大幅上涨，导致现货价格为负。期货贴水，说明现货的供应越紧张。相反，期货升水，说明现货供应充足。因此，此时期货与现货价格的关系（升贴水），体现了短期内现货供求关系的预期，反应了\uline{投资者的情绪与预期}。并且由于便利收益的主观性，价格由投机者的决定，此时价格反应市场的情绪与流动性水平。并且由于无套利条件被打破，期货价格并不能发现现货价格。

此时也可以将便利收益，放入做空成本中考虑，即不愿意放弃现货，所支付的成本，因此应有$F_{t,T}< S e^{r(T-t)}$。另外，对于现货的卖空限制，以及其他约束，实质上也打破了期货现货的联动关系，并使得期货无法发挥风险管理功能。

\subsubsection*{价格发现}

价格发现是金融市场的核心功能质疑，通过市场的交易（标准化的二级市场），交易价格充分反映了市场关于标的的信息，并非期货市场独有。价格是信息的反应，并无“对、错”。期货市场（特别是商品期货），由于合约的标准化、交易机制的透明、交易成本低，因此其相比现货市场信息传递效率更高，这就是\uline{期货的价格发现功能}。期货信息将原本信息不透明、价格更新频率低的商品现货贸易，变为信息透明、价格更新频率高的金融交易，此过程被称为“商品金融化”（Commodities financialization）。

\uline{期货（远期）价格不能发现“未来现货价格”}，它只是市场参与者基于当下信息的判断。理论上，当无套利条件精准成立，仓储、利率等条件不存在不确定性时，期货（远期）价格与预期完全无关，仅传递与现货相同的信息。但在现实中，无套利条件难以成立（如商品期货或融资约束较高的金融期货），又如仓储、利率等存在随机性时，期货（远期）价格中包含预期的因素。具体而言为对便利性收益、仓储成本与资金成本的预期。并且由于期货到期交割的机制，庞大的现货（场外市场）对期货价格形成一定的约束。

\subsection{\tops{$F_{t,T}$}与\tops{$\E_t[S_T]$}}

在现实测度下，$F_{t,T}$与$\E_t[S_T]$两者相差风险溢价，但由于风险溢价的不确定性，因此两者关系并不确定：
\begin{equation*}
    F_{t,T} = \E_t[S_T] - \text{风险溢价}
\end{equation*}

从现货角度出发，假设在现实测度下，应有$\E_t[S_T] = S_t e^{(\mu-q)(T-t)}$，此时$\mu$预期收益率，$r$为无风险收益率，两者之差$\mu-r = rp$为\uline{风险溢酬}。从期货角度出发，有$F_{t,T} = S_t e^{(r-q)(T-t)}$，由于购买期货无需占用资金（假设保证金可获得无风险收益），所以不要求无风险收益，并且投资期货并不需要支付仓储成本，无法获得红利，仅要求对其承担的未来不确定性风险，有对应的风险补偿（风险溢酬）。因此投资期货，其预期收益率为$rp = \mu-r$，单纯为\uline{风险溢酬}或风险补偿，此为期货的风险转移，或风险管理功能。因此在风险中性世界中，由于投资者不要求风险补偿，因此期货的预期收益率为零。
\begin{equation*}
    \frac{\ln\E_t[F_{T,T}] - \ln F_{t,T}}{T-t} = \frac{\ln\E_t[S_T] - \ln F_{t,T}}{T-t} = \mu - r = rp
\end{equation*}

在风险中性下，投资者不要求风险补偿，仅要求无风险收益，即$\mu=r$。因此当前的期货价格，为未来到期时刻现货价格的风险中性期望：
\begin{equation*}
    \rnE_t[S_T] = S_t e^{(r-q)(T-t)} = F_{t,T}
\end{equation*}

可以得到，对于未来股票价格的风险中性测度与现实测度的期望差，即为风险溢价：
\begin{equation*}
    F_{t,T} = \rnE_t[S_T] = \E_t[S_T] - \text{风险溢价}
\end{equation*}

对于风险溢酬的符号，取决于资产风险与系统性风险的相关性。若资产风险与系统性风险（无法分散的风险）正相关，即当市场变坏时，资产收益下降，当市场变好时，资产收益率上升，因此为其为“坏”资产，卖得便宜，收益率更高。相反，若与系统性风险为负相关，则为“好资产”，因为能对冲系统性风险，收益率应更低。

具体而言如果标的资产$S$\textbf{系统性风险为正}（与系统性风险正相关），则要求有\textbf{正的风险溢酬}。承受了更高的系统性风险，因此应有超过无风险收益的更高回报，则有到期收益率大于无风险收益率$\mu>r$。相反，如果标的资产$S$\textbf{系统性风险为负}（如VIX系统性风险为负），则应有\textbf{负的风险溢酬}。标的资产能对冲风险，减少系统性风险敞口，此时风险溢酬为负，因此只能获得小于无风险收益的回报，因此到期收益率小于无风险收益率$\mu<r$。假设在T时刻期货价格期望为$\E_t[S_T] = \E_t[F_{T,T}] = 2000$，由于标的资产的系统风险为正，对持有者不利，其要求500元的正的风险溢价为额外承担风险的补偿，那么此时$S_t$应为1500元。
...
\begin{figure}[H]
    ...
    \label{fig:M2}  
    \end{subfigure}
    \caption{现实测度下$F_{t,T}$与$\E_t(S_T)$}
\end{figure}


\subsection{\tops{$F_{t,T}$}、\tops{$S_t$}与\tops{$\E_t[S_t]$}}

如上述章节所述，从期货价格出发$F_{t,T}$，期货价格与现货价格$S_t$的关系由资产的\uline{净持有成本}，无套利关系决定。而期货价格$F_{t,T}$与未来现货价格的期望$\E_t[S_T]$的关系，则由资产的\uline{风险溢酬}决定。另外虽然现货价格可以从市场交易信息中得到，但由于期货市场交易成本较低，反应较为灵敏迅速，价格变化一般都由期货市场传递至现货市场。在t时刻，期货与现货以及其现实测度期望关系如下图所示（金融期货）：

\begin{figure}[H]  
    \centering
    \begin{tikzpicture}
        \draw [<->,thick] (1,0) node [left,xshift=-0.4em]{$S_t$} -- (6,0) node [right]{$\E_t[S_T]$} node [midway,below,yshift=-0.4em]{\small $S_t e^{(\mu-q)(T-t)} = \E_t[S_T]$};
        \draw [<->,thick] (1,3) node [left]{$F_{t,T}$} -- (6,3) node [right]{$\E_t[F_{T,T}]$} node [midway,above,yshift=0.4em]{\small $F_{t,T} e^{(\mu-r)(T-t)} = \E_t[F_{T,T}]$};
        \draw [<->,thick] (0.5,0.5) -- (0.5,2.5) node [midway,left,xshift=-0.4em]{\small $F_{t,T}=S_t e^{(r-q)(T-t)}$};
        \draw [<->,thick] (6.5,0.5) -- (6.5,2.5) node [midway,right,xshift=0.4em]{\small $F_{T,T}=S_T$};
    \end{tikzpicture}
    \caption{四者之间的关系}
\end{figure}
...
\begin{itemize}
    ...
    \item 赎回权（Call option）：该期权规定，在一定期限内，如果公司股票的价格在若干个交易日内满足赎回条件，公司有权按照\text{赎回价格}，赎回公司的剩余可转债。
...
\textbf{股利}（Dividend），又称为股息或红利，是上市公司对股东的投资回报。在上市公司的税后利润中，其分配顺序如下：
\begin{enumerate}
    \item 弥补以前年度亏损
...
一般而言，\uline{股息}是股东定期按一定的比率（特别对于优先股而言）从上市公司分取的盈利，股息的比率相对固定。优先股股东有股息收入作为保障，一般不参与公司分红。而\uline{红利}则是在上市公司分派股息之后按持股比例向股东分配的剩余利润，因此只有在公司获取利润时才能分配红利，数额通常不固定，随着公司每年可分配盈余多少上下浮动。也有人将优先股的收益称之为股息，而普通股的收益称之为红利。一般将股息与红利统称为股利：
\begin{equation*}
    \text{股利} = \text{股息} + \text{红利}
\end{equation*}

由于要在获得利润后才能向股东分派股息和红利，因此上市公司一般是在公司营业年度（或半年度）结算以后才从事这项工作。A股上市公司一般选择会在半年度或年度进行分红，选择年度进行分红的企业较多。半年报截止与8月底，因此9月为半年报分红的高峰，而年报截止到4月底，因此\uline{5月是年报分红的高峰}。
...
上市公司发行股票所募集资金后会分为两个部分，股本（实收资本，A股股票面值大多为每股一元）与资本公积。股本即股票面值乘以股份数的部分，超过股本的部分为\uline{资本公积}。假如上市发行价为5元，面值1元为股本，而超过面值的4元称为资本公积。而\uline{盈余公积}就是而当公司获得收益之后，从税后利润提取，留存在公司内部。同时税后利润在提取盈余公积金以及向投资者分配利润后，继续留存在企业内的利润就称之为未分配利润。从盈余公积以及未分配利润，转化为股本为\uline{送股}，而\uline{转股}则为将资本公积转化为股本。
...
\uline{现金股利}是上市公司将可分配利润以现金的形式支付给股东，但总股本大小不变，因此股东权益减少。对应每股所对应的价值（每股净资产=归属母公司股东权益/总股本）减少，因此价格应相应下调，这一过程叫\textbf{除息}。\uline{股票股利}或送股是上市公司将可分配利润留在公司里，发放股票作为股利，实质为将未分配利润或盈余公积转股本，两者本都属于股东权益。因此送股后公司的资产、负债、股东权益的总额结构并没有发生改变，但总股本增大了，导致每股净资产降低了，股价也相应降低。从股票价格中剔除这部分因素，称之为\textbf{除权}。

送股和转增是比较常见的总股本发生变化导致除权的情况，其他还有如增发、配股回购注销等。\uline{增发}即增发新股，即上市公司再一次发行新股，进行再融资。发行对象可以是指定投资者（大股东或机构），或全部投资者额外发行股份募集，发行价格应不低于公告招股意向书前20个交易日公司股票均价或前一个交易日的均价。而定向发行则为上市公司采用非公开方式，向特定对象发行股票的行为（发行对象不超过10名）。\uline{配股}为上公司按一定比例向现有股东发行新股，属于再融资，股东需要按配售价与配售数量缴纳配股款，不同于分红派息。
...
\begin{example}
    以平安银行（000001.SZ）为例，2022年7月22日，10股派2.28元（即每股派0.228元现金红利）。
    
    CSMAR“日市场回报率文件”中显示，22日当日不复权收盘价为12.81，昨收为12.78，那么此时计算日收益率为0.002347。若使用前一日21日收盘价13.01，此时计算的收益率为-0.015373。而21日收盘价13.01与22日的昨收12.78之差，正为现金红利0.23元。CSMAR数据库中，“考虑现金红利再投资”与“不考虑现金红利再投资”收益率分别为0.002152与-0.1537。

    CSMAR“股票历史日行情信息表(后复权)”中，22日收盘价为1975.95，而21日收盘价为1971.32，收益率为0.00235。万得中，7月22日不复权收盘价12.81，前一日收盘价13.01，万得显示当日涨跌幅为-1.54\%。而对于后复权而言，当日收盘价为1459.52，前一日收盘价为1456.10，万得显示后复权涨跌幅为0.23\%。

    原始行情上CSMAR与WIND计算结果相同，虽然两者在后复权的价格上存在差异，但由后复权计算的涨跌幅相同。
\end{example}

\begin{note}
    关于收益率的计算：
    \begin{itemize}
        \item 收益率需要使用后复权价格进行计算，与真实收益率不相等但近似
        \item 使用不复权价格，使用当天的”PreClose“或“昨收”字段（为经过除权）与当天收盘价格计算的收益率，即CSMAR数据库中"ChangeRatio"字段，计算方式为：今收/昨收-1
        \item 如上两种计算方式，结果与CSMAR数据库中“考虑现金红利再投资的日个股回报率”近似
        \item 若直接使用t-1日的收盘价，与当天收盘价所计算的收益率，与CSMAR数据库中“不考虑现金红利再投资的日个股回报率”近似或相等
    \end{itemize}
\end{note}
...
以美元兑人民币为例，通常表示为$\text{USD/CNY} = 6.7$，或也经常表示为USDCNY或USD：CNY，称为USD to CNY或USD against CNY。即1美金（每美金），兑换6.7人民币，即CNY per USD，（￥6.7/\$），用英文表述为quoted as quote currency units per base currency。在USDCNY中，在前的货币称为Base currency（每），在后的称为Quote currency（计价货币）或称为Local currency（本币）。
\begin{equation*}
...
为了更好理解，若将苹果作为一种货币，假设苹果价格为2元/个（苹果），即有苹果：人民币为2。
此时人民币为本币（Local currency）或计价货币，而苹果为外币。在外汇交易中，显然做多苹果，需要卖出的人民币，买入苹果，则希望苹果涨价。以做多USD/CNY为例，假设当前为6.7，若做多USDCNY，即希望USD上涨。卖出本币CNY，并买入外币USD，一个月后上涨至6.8，即每单位USD可兑换的CNY变多，即Base currency USD对CNY上涨，从而获利。若做空USD/CNY，那么希望USD跌。

\subsection{利率平价}

利率平价（Interest rate parity）可分为以下两种：
\begin{itemize}
    \item Covered Interest Rate Parity（CIP）：有抛补利率平价，利用远期合约锁定远期汇率，对冲风险
    \item Uncovered Interest Rate Parity（UIP）：无抛补利率平价，不适用远期合约单对冲风险，所以是uncovered，根据有效市场假说，应该不能获得超额收益
\end{itemize}

对于CIP，使用远期汇率对冲风险。假设即期汇率S，远期汇率F（形式均为FOR/LOC，即每单位FOR外币，兑换多少LOC本币）:
\begin{equation*}
    (1 + i_{L}) = \frac{F_{F/L}}{S_{F/L}} (1+ i_{F})
\end{equation*}

对上述公式，若取对数则有，并使用$x=\log(1+x)$进行近似：
\begin{gather*}
    \log(1+i_L) = \log(F_{F/L}) - \log(S_{F/L}) + \log(1+i_F) \\
    i_L \approx \log(F_{F/L}) - \log(S_{F/L}) + i_F
\end{gather*}

因此应有：
\begin{equation*}
    \log(F_{F/L}) - \log(S_{F/L}) \approx i_L - i_F 
\end{equation*}

两国利差，如中国名义（Nominal）利率为3\%，而美国利率为1\%，那么应该卖出美金，买入人民币，投资中国债券，从而赚取利差。那么未来应该人民币贬值2\%（USDCNY上涨2\%），否则就存在套利机会。即投资高利率国所获得的额外收益，将由该国汇率贬值抵消。
...
因此对于CIP而言，应有如下近似关系，即利差应与汇率变化相等：
\begin{equation*}
    (i_{L}-i_{F}) \approx \frac{F_{F/L}-S_{F/L}}{S_{F/L}}
\end{equation*}

对于UIP而言，不用远期锁定未来汇率，假设UIP成立，未来汇率的期望应由以下关系得到：
\begin{equation*}
    (1 + i_{L}) = \frac{\E(S_{t+1})}{S_t} (1+ i_{F})
\end{equation*}

若CIP与UIP都同时成立（不考虑风险溢酬），那么：
\begin{equation*}
    \E (S_{t+1}) = F_{F/L}
\end{equation*}

\begin{example}
    假设当前GBP/USD汇率为1.35，并且美国的利率为1.1\%，英国的利率为3.25\%，那么根据CIP远期汇率应有持有1单位本国货币（USD），投资本国债券所获得的收益，即$1+1.1\%$。应与1美金兑换成英镑并持有，$\tfrac{1}{S_{F/L}} \times (1+3.25\%)$，最终将其通过远期汇率兑换会美金，由于使用远期合约进行锁定，那么两者应有CIP平价关系：
    \begin{equation*}
        (1+1.1\%) = \frac{1}{S_{F/L}} \times (1+3.25\%) \times F_{F/L}
    \end{equation*}

    此时可以看到，远期汇率约为$1.32$，即英镑应贬值约$(1.32-1.35)/1.35=2.22\%$，近似于两国利差：
    \begin{equation*}
        F_{F/L} = 1.35 \times \frac{(1+1.1\%)}{(1+3.25\%)} \approx 1.32
    \end{equation*}
\end{example}

\begin{example}
    假设当前AUD/USD = 0.75，澳洲与美国两年期的利率收益分别为$3\%$与$2\%$。汇率表示为AUDUSD，可以看到此时美元为本币，并将澳元理解为需要购买的商品（外币）。那么此时持有成本为占用资金所需要支付的利息成本$e^{1\% \times 2}$，而持有收益为投资外币所获得的澳洲利率收益$e^{3\% \times 2}$。因此两年后的远期汇率应有：
    \begin{align*}
        F_{0,2} &= S_0 + \text{持有成本} - \text{持有收益} \\
        &= 0.7500 \times  e^{1\% \times 2} \times e^{-3\% \times 2} \\
        &= 0.7206
    \end{align*}

    若真实交易的远期价格大于0.7206，如0.7600。那么在未来澳元更贵，那么应该持有澳元，并在两年后卖澳元。具体应该卖出AUDUSD两年后的远期合约（届时卖出AUD，买入USD），借入1\%两年的1000美元，即换得澳元$1000/0.75 = 1333.33$，投资3\%澳币并持有两年，两年后得到澳元$1415.78$，按照远期0.76的价格换算为美元，得到$1075.99$美元，扣除美元资金成本$1000\times e^{0.02} = 1020.20$，最终得到收益为$55.79$美元或$73.41$澳元。

    相反，若真实交易的远期价格小于0.7206，如0.7000，此时在两年后未来（远期）澳元更便宜，美元更贵。应该持有美元，并在两年后卖出美元。具体操作与上述相反，买入AUDUSD两年后的远期合约（届时买入AUD，卖出USD），借入3\% 1000澳元两年，转换为美金，并投资两年，$1000 \times 0.75 \times e^{0.02} = 765.15$，在到期后根据远期合约，兑换回澳元为$765.15/0.7=1093.07$，支付澳元资金成本$1000 \times e^{0.06} = 1061.84$，得到利润为$31.23$澳元或$21.86$美元。
\end{example}

若考虑连续复利收益，汇率同样为FOR/LOC，$r_L$为本币的利率，而$r_F$为外币的利率，此时根据期货现货持有成本框架，应有：
\begin{equation*}
    F_{t,T} = S_t e^{(r_L-r_F)(T-t)}
\end{equation*}

取对数则有CIP关系：
\begin{equation*}
    \frac{\ln F_{t,T} - \ln S_t}{T-t} = r_L-r_F
\end{equation*}

同样，UIP关系应有（不考虑风险溢酬），而在许多实证中近似成立，并非是由于无套利关系成立，而大多是由于两个国家之间，如发达经济体之间，风险溢酬相减之后较小，风险溢酬表现不明显。而当两个经济发展差距较大的情况下，风险溢酬显著，使得UIP不成立。
\begin{equation*}
    \frac{\ln \E_t [S_T] - \ln S_t}{T-t} = r_L-r_F
\end{equation*}

可以从两个角度理解：从利率角度，若投资者投资美国债券，美国债券价格上涨，收益率下跌，最终使得套利机会消失；若从汇率角度，在未来应该有AUDUSD下跌，幅度相当于两国利差，使得套利机会消失。
