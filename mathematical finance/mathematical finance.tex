...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/options-profit.png}
    \caption{期权收益}
    \label{fig:opt-profit}
\end{figure}
...
期权是高度量化的工具，衡量每一个因素对期权价格变化的影响，在数学中即为求导。令期权价格价格为V，那么期权价格对标的价格的一阶导为Delta，对标的价格的二阶导为Gamma，对于波动率的一阶导为Vega，对于期限的一阶导为Theta，对于无风险利率的一阶导为Rho。
\begin{equation*}
...
可以将Delta理解为期权价格与标的价格的贴合程度，以看涨期权为例，当为深度实值时，贴合程度最好，标的价格变化1单位，期权价格变化1单位。即虚值贴合差，对股价变动不敏感，而实值贴合好，对股价变动敏感。如下图所示，随着到期时间不断减少，期权价格曲线将越来越贴合期权内在价值。那么不同在值程度期权的Delta也将随着期限的衰减，差异不断加大。实值期权Delta将加速趋近1，而虚值期权将加速趋近0，平值期权变化较小。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/delta_surf.png}
    \caption{看涨期权多方Delta}
    \label{fig:delta_surf}
\end{figure}
...
若以标的资产价格为横轴，期权价格为纵轴，如上图期权价格与标的价格所示，Delta即曲线切线的斜率，即一阶导。而那么Gamma就是曲线的二阶导，为Delta的变化速度，或曲线弯曲程度或加速度。

做多期权，有正的Gamma，能使得价格变化对投资者有利。即当价格上涨的时候，对投资者有利，变化越来越快，盈利越来越多。相反，当价格下跌时，对投资者不利，变化越来越慢，损失越来越小。而作为做市商等，卖期权，或做空期权，则有负的Gamma。此时与期权投资者相反，价格上涨时，盈利减慢，而价格下跌时，损失增大。正Gamma为凸函数（Convex，下凸），负Gamma为凹函数（Concave，上凸），另外可以看到在平值点的Gamma最大。看涨期权与看跌期权的Gamma相同。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/gamma_surf.png}
...
Vega体现的为期权价格对波动率的敏感程度，即波动率变化$1\%$对期权价格的影响。对于期权的多头，其Vega为正，意味着隐含波动率的正向变化对其有利。看涨期权与看跌期权的Vega相同。

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/vega_surf.png}
...
下图期权的时间价值曲线中，随着期限的减少，期权的时间价值在加速衰减。因此对于期权的多方，其时间价值在不断衰减，有Theta为负。相反，空方则Theta为正。注意：对于Thtea多空符号相同，但值不同。
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{fig/theta_surf.png}
    \caption{看涨期权多方Theta}
    \label{fig:}
\end{figure}
...
另外，由BSM偏微分方程可知：
\begin{equation*}
    \frac{\partial V}{\partial t} + r S_t \frac{\partial V}{\partial S} + \frac{1}{2} \sigma^2 S^2 \frac{\partial^2 V}{\partial S^2} = rV 
\end{equation*}

改写为希腊值的形式：
\begin{equation*}
    \Theta + r S_t \Delta + \frac{1}{2} \sigma^2 S^2 \Gamma = rV
\end{equation*}

当Delta被完全对冲，即$\Delta=0$。为了讨论方便起见，假设无风险利率为0，因此有：
\begin{equation*}
    \Theta + \frac{1}{2} \sigma^2 S^2 \Gamma = 0
\end{equation*}

那么可以看到$\Theta=\frac{\partial V}{\partial t}$与$\Gamma=\frac{\partial^2 V}{\partial S^2}$的制约关系。即对于期权的多头，其Gamma为正，但同时Theta为负，对于空头则相反。
