...
定价思路为由于在签订远期合约时，双方签订在未来$T$时刻的交割价格（Delivery price），应基于双方公平的原则（Zero net market value at entry），使得远期合约签订之时其价值（Forward value）为零或$f(0,S_0)=0$。因此得到远期（期货）价格（Forward price）$F_{t,T}$，即在t时刻，使得远期合约价值为零的合理交割价格（可以理解为在t时刻不断的签订新的远期合约）。但在该合约签订之后，随着标的资产价格的变化，该合约的价值（$f$）将不断发生变化。
\begin{itemize}
...
USD/CNY约为6.7，即美元兑人民币为6.7（￥6.7/\$，quoted as quote currency units per base currency）。从字面理解即1美金（每美金），兑换6.7人民币，也可称为CNY per USD。也经常表示为USDCNY或USD：CNY，称为USD to CNY或USD against CNY。在前的货币称为Base currency（每），在后的称为Quote currency或称为Local currency。
