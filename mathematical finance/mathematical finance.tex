...
\begin{itemize}
    ...
    \item 做空机制：现货做空机制不完善，难以进行融券，即使能融券成本也较高。使得套利投资者在现货被高估的情况下，难以通过卖出现货（卖出开仓）买进期货进行\textbf{反向套利}（Reversed cash and carry arbitrage；买入现货卖出期货为\textbf{正向套利}）。造成现货与期货市场的分割，无法达到一体化的预期。
