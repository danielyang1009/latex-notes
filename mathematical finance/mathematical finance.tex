...
利息成本无可避免，若持有成本为$c$，那么应有$F_{t,T} = S_t e^{(r+c)(T-t)}$。其中对于商品期货，便利收益指实际持有持有现货带来的收益，一般认为由于持有现货有两点优势：可从短期商品的紧缺中获利，也可保持生产线的运转。由于在$t$时刻，如现货价格和净持仓成本都为已知，则可确定期货价格。相反已知期货的价格与净持仓成本，则可确定现货价格，此为期货的价格发现功能。两者为相对价格关系，基于无套利条件得到。

\subsubsection*{基差与期货升贴水}

对于商品期货而言，基差（Basis）指现货与远期价格之差（现货价格Spot price有时又被称为Cash price）。通常基差指期限短减去期限长，而升贴水指对于期货而言，期货价格（期限长）大于现货价格（期限短）为升水，反之亦然。对于基差有：
\begin{equation*}
...
\begin{itemize}
    ...
    \item 负基差：现货价格低于期货价格，期货升水（Contango，支付交易延期费），净持仓成本为正。此时的市场被称之为正向市场或正常市场（Normal market），即期限较短的合约价格低于期限较远的合约。之所以称之为正向市场，由于大多数\uline{商品期货储藏成本较大，大于便利收益，使得期货升水}。
\end{itemize}
...
\begin{itemize}
    \item 中性交易：机构做空需求，市场中性策略或Alpha策略基金都需要做空股指期货，即支付“贴水”成本给提供流动性的做多者。并且中小盘股相较于大盘股更容易做Alpha策略，因此对中小盘的做空需求更高，因此IC与IM的贴水要比IH与IF高。【待核实】而在市场早期时，主要流行打新？因此卖50，造成50贴水。
...
从现货角度出发，假设在现实测度下，设$\mu$为现货$S$的到期收益率，则应有$\E_t[S_T] = S_t e^{\mu(T-t)}$，此时$\mu$预期收益，$r$为无风险收益率，两者之差$rp = \mu-r$为风险溢酬。对于风险溢酬的符号，取决于资产风险与系统性风险的相关性。若资产风险与系统性风险（无法分散的风险）正相关，即当市场变坏时，资产收益下降，当市场变好时，资产收益率上升，因此为其为“坏”资产，卖得便宜，收益率更高。相反，若与系统性风险为负相关，则为“好资产”，因为能对冲系统性风险，收益率应更低。
...
从期货角度出发，同样在现实测度下，因为购买期货无需占用资金（假设保证金可获得无风险收益），所以不要求无风险收益，并且投资期货并不需要支付仓储成本，无法获得红利。仅要求对其承担的未来不确定性风险，有对应的风险补偿（风险溢酬），因此其收益率为$rp = \mu-r$，单纯为风险溢酬，即$\E_t[F_{T,T}] = F_{t,T} e^{(\mu-r)(T-t)}$。同时可知，在风险中性世界中，期货的预期收益率为零。

\subsection{\tops{$F_{t,T}$}、\tops{$S_t$}与\tops{$\E_t[S_t]$}}

如上述章节所述，从期货价格出发$F_{t,T}$，期货价格与现货价格$S_t$的关系由资产的\uline{净持有成本}，无套利关系决定。而期货价格与未来现货价格的期望$\E_t[S_T]$的关系，则由资产的\uline{风险溢酬}决定。
