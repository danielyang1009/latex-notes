...
欧式看涨与看跌期权具有执行价格（行权价）和期限时，他们的隐含波动率是一样的。

这说明欧式看涨期权的波动率微笑，与相同期限的欧式看跌期权的波动率微笑应该相同。

已知对于相同期限（$T$）、相同执行价格（$K$）的看涨看跌期权，根据PCP关系有：
\begin{equation*}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{fig/vol_smile_skew.png}
    \caption{波动率微笑与波动率偏斜}
    \label{fig:vol_smile_skew}
\end{figure}
