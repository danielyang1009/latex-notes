...
对数压缩了数据的尺度，使得数据在较小时更敏感。对比上证指数与上证指数取对数后，在取对数之前可以看到05-07年大牛市从998到6124取得了600\%的收益。而在取了对数之后，可以清晰看到，90年代末期的牛市从95到1558点，得到1500\%的收益率。即随着指数数值越来越大，绝对数值的变化越来越大，使得难以观察其相对数值的变化。以2为底取对数为例，$\log_2 2 =1$，$\log_2 4 =2$，$\log_2 8 =3$，可以看到取对数后每单位的变化，对应着相同百分比的变化。
...
而对于不同资产在截面上的收益率，则应使用百分比收益率，具有截面可加性。以两个资产为例，投资资金为$m_1$与$m_2$，期末百分比收益率为$r_1$与$r_2$，那么此时有：
\begin{align*}
