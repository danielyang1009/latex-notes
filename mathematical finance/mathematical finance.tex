...
\subsection{\tops{$S_t$}、\tops{$F_{t,T}$}与\tops{$\E_t[S_t]$}}
...
\begin{figure}[H]  
    \centering
    \begin{tikzpicture}
        \draw [<->,thick] (1,0) node [left,xshift=-0.4em]{$S_t$} -- (6,0) node [right]{$\E_t[S_T]$} node [midway,below,yshift=-0.4em]{\small $S_t e^{\mu(T-t)} = \E_t[S_T]$};
        ...
        \draw [<->,thick] (0.5,0.5) -- (0.5,2.5) node [midway,left,xshift=-0.4em]{\small $F_{t,T}=S_t e^{r(T-t)}$};
