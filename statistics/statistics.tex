...
$a_t = r_t - \mu_t$为均值方程的残差，则可以用平方序列$a_t^2$来检验\textbf{条件异方差性}，即所谓的\textbf{ARCH效应}，即为残差平方之间的序列相关性，或白噪声检测。第一种方式，可以使用\textbf{Ljung-Box}统计量$\{Q(m)\}$应用于序列$\{a_t^2\}$，参见Mcleod和Li（1983）。该检验的原假设是$\{a_t^2\}$序列的前$m$个间隔的ACF值都为零。第二种方式，可以使用Engle（1982）中的拉格朗日乘子检验。对于\textbf{拉格朗日乘子检验}（Lagrange Multiplier Test，LM检验），等价于对如下线性回归中用F统计量检验$\alpha_i=0\;(i=1,\dots,m)$，即原假设有：
\begin{equation*}
...
由于$\sigma_t$可以通过$F_{t-1}$信息集获得，为已知量。因此可以证明：
\begin{align*}
...
将均值模型与波动率模型拆开估计，与两者联合估计并不相同。若将其拆开估计，即先使用ARMA(p,q)对$r_t$建模，确定最优参数p和q，而后使用ARMA模型残差对其进行GARCH建模。由于此时因变量或被解释变量为残差，因此相当于使用均值为0的均值模型，与GARCH模型进行联合估计。即抛弃了联合估计中异方差对收益率序列的影响。
