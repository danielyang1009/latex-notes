...
条件似然函数（相乘），最大化条件似然函数，等同于最大化对数，因此可以将其写为对数似然函数（相加）
...
若$\{r_t\}$为\textbf{弱平稳}序列，满足$r_t = \mu + \sum_{i=0}^{q}\psi_i a_{t-i}$，其中$\phi_0=1$，$\{a_t\}$为均值为0的独立同分布变量序列，对于$l>q$，则有$\hat{p}_l$渐进的服从如下正态分布，称为Bartlett公式：
\begin{equation*}
...
其中$\mu$为$r_t$的均值，$\pi_0=1$，并且$\{a_t\}$为均值为零的独立同分布随机变量序列，即$\{a_t\}$为白噪声序列。可以认为$a_t$表示该时间序列$t$时刻出现的新息（Innovation）或扰动（Shock）。其中$r_t$系数被称为$\psi$-权重，若$r_t$为弱平稳，则可以利用$\{a_t\}$的独立性得到：
\begin{equation*}
...
其中$\{a_t\}$为均值为0，方差为$\sigma_{a}^2$的白噪音，此时由于$r_t$为因变量，$r_{t-1}$为自变量，因此如上模型也被称为自回归模型（Autoregressive model，AR），由于与自身滞后一阶自回归，因此称为一阶自回归模型或AR(1)模型。在过去收益率$r_{t-1}$已知条件下。有其均值与方差（波动来源于白噪声）有：
\begin{align*}
    \E(r_t \given r_{t-l}) &= \phi_0 + \phi_1 r_{t-1} \\
    \Var(r_t \given r_{t-l}) &= \E\left[\left(r_t - \E(r_t \given r_{t-1}) \right)^2\right] \\
    & = \E \left(a_t^2\right) = \Var(a_t) = \sigma_{a}^{2}
\end{align*}
...
\begin{equation*}
    r_t = \mu(1-\phi_1) + \phi_1 r_{t-1} + a_t = \mu + \phi_1 (r_{t-1}-\mu)+ a_t 
\end{equation*}
...
$r_t$为两个扰动项$a_t$与$a_{t-1}$的加权平均，因此模型称为1阶MA模型，简称MA(1)。其中$\{a_t\}$为白噪声序列，且$c_0$为常数：
\begin{equation*}
...
对于给定$t-1$时刻，已知信息集合$F_{t-1}$时，$r_t$的条件均值与条件方差为：
\begin{align*}
...
$a_t = r_t - \mu_t$为均值方程的残差，则可以用平方序列$a_t^2$来检验\textbf{条件异方差性}，即所谓的\textbf{ARCH效应}，即为残差平方之间的序列相关性，或白噪声检测。第一种方式，可以使用Ljung-Box统计量$\{Q(m)\}$应用于序列$\{a_t^2\}$，参见Mcleod和Li（1983）。该检验的原假设是$\{a_t^2\}$序列的前$m$个间隔的ACF值都为零。第二种方式，可以使用Engle（1982）中的拉格朗日乘子检验。对于拉格朗日乘子检验（Lagrange Multiplier Test），等价于对如下线性回归中用F统计量检验$\alpha_i=0\;(i=1,\dots,m)$，即原假设有：
\begin{equation*}
...
第一个尝试给波动率（$\sigma_t$，收益率的条件标准差）建模并提供系统框架的是由Engle（1982）提出的ARCH模型。基本思想为：
\begin{itemize}
...
具体而言，ARCH(m)模型如下所示。其中$\{\varepsilon_t\}$为均值为$0$，方差为$1$的独立同分布随机变量序列：
\begin{gather*}
    a_t = \sigma_t \varepsilon_t \\
    \sigma_t^2 = \alpha_0 + \alpha_1 a_{t-1}^2 + \dots + \alpha_m a_{t-m}^2
\end{gather*}

由于$a_t = r_t - \E(r_t \given F_{t-1})$，因此$\E(a_t) = \E(r_t) - \E[\E(r_t \given F_{t-1})] = \E(r_t) - \E(r_t) = 0$。并且有$\E(a_t \given F_{t-1}) = \E(\sigma_t \varepsilon_t \given F_{t-1}) = \varepsilon_t \E(\sigma_t \given F_{t-1}) = 0$。
    
由于$\varepsilon_t$独立，因此有$\Var(\varepsilon_t) = \Var(\varepsilon_t \given F_{t-1}) = \E(\varepsilon_{t}^{2} | F_{t-1}) = 1$。对于条件方差有性质\uline{提取已知量}，即对于已知量，可以直接从条件期望中提出。具体而言，若$X$是$\mathcal{H}$可测的，应有：
\begin{equation*}
    \E[X Y\given \mathcal{H}] = X \E[Y \given \mathcal{H}] 
\end{equation*}

由于$\sigma_t$是可以通过$F_{t-1}$信息集获得，为已知量。因此可以证明：
\begin{align*}
    ...
    = \sigma_{t}^{2} \\
    &= \alpha_0 + \alpha_1 a_{t-1}^2 + \dots + \alpha_m a_{t-m}^2
\end{align*}

同时其$a_t$的无条件方差为：
\begin{align*}
    \Var(a_t) &= \E\left[(a_t - E[a_t])^2\right] \\
    &= \E\left[ a_t^2 \right] = \E\left[ \E(a_t^2 \given F_{t-1}) \right]
    = \E\left[ \E(\sigma_t^2 \varepsilon_t^2 \given F_{t-1}) \right] \\
    &= \E\left[ \sigma_t^2 \E( \varepsilon_t^2 \given F_{t-1}) \right] = \E\left[ \sigma_t^2 \right] \\
    &= \E\left[ \alpha_0 + \alpha_1 a_{t-1}^2 + \dots + \alpha_m a_{t-m}^2 \right]
\end{align*}

\subsubsection{ARCH模型的性质}

对于ARCH(1)模型而言有，即只有一阶滞后项时：
\begin{gather*}
    a_t = \sigma_t \varepsilon_t \\
...
\begin{equation*}
    ...
    = \E\left[ \alpha_0 + \alpha_1 \sigma_{t-1}^{2} \right]
    = \alpha_0 + \alpha_1 \E\left[\sigma_{t-1}^{2}\right]
\end{equation*}

$a_t$是平稳过程，并有$\E(a_t) = 0$，因此$\Var(a_t) = \Var(a_{t-1}) = \E(a_{t-1}^{2})$。因此有如下关系，并且由于方差为正，因此$0 \leq\alpha <1$：
\begin{equation*}
...
最大化上式得到的估计称之为\uline{条件最大似然估计}（MLE），同时最大化条件似然函数\uline{等价于}最大化其对数，将条件似然函数（相乘），改写为对数似然函数（相加）:
\begin{equation*}
...
\begin{equation*}
    \mcl(a_{m+1},\dots,a_T \given \bm{\alpha},a_1,\dots,a_m) =
    \bm{-} \sum_{i=m+1}^{T} \left[\frac{1}{2} \ln(\sigma_{t}^{2}) + \frac{1}{2} \frac{a_{t}^{2}}{\sigma_{t}^{2}} \right]
\end{equation*}

\subsection{GARCH模型}

Bollerslev（1986）年提出了对于ARCH模型的推广形式，称之为\textbf{广义ARCH模型}（Generalized Autoregressive Conditional Heteroskedasticity，GARCH）。对于对数收益率序列$r_t$，令$a_t = r_t - \mu_t$为$t$时刻的新息或扰动，则称$a_t$服从GARCH(m,s)模型，若$a_t$满足下式：
\begin{gather*}
    a_t = \sigma_t \varepsilon_t \\
    \sigma_{t}^{2} = \alpha_0 + \sum_{i=1}^{m} \alpha_i a_{t-i}^2 + \sum_{i=1}^{s} \beta_j \sigma_{t-j}^{2}
\end{gather*}

当$s=0$时，该模型就简化为ARCH(m)模型，同时将$\alpha_i$与$\beta_j$称为\uline{ARCH参数}与\uline{GARCH参数}。
