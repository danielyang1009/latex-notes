...
\begin{itemize}[leftmargin=4em]
    ...
    \item 波动率为一个时间窗口内（一般为年，252天交易日），日频连续复合收益率或对数收益率（$\ln\frac{S_{t+1}}{S_{t}}$）标准差进行年化得到。
...
\begin{itemize}
    ...
    \item 同理，对于看跌期权，其下边界为$P_t \geq \max \left[ Ke^{-r(T-t)} - S_t,0 \right]$
\end{itemize}
...
波动率的数学定义为：假设$r_t$为一个资产的收益率，一般使用对数收益率（连续复利收益率）时间序列，令$t=1,2,3,\dots,T$，则有样本方差为：
\begin{align*}
    \sigma^2 &= \frac{1}{T-1} \sum^T_{t=1} (r_t - \bar{r})^2 \\
    &= \frac{1}{T-1} \left( \sum^T_{t=1} r_t^2 - 2\bar{r} \sum^T_{t=1} r_t + \sum^T_{t=1} \bar{r}^2 \right) \\
    &= \frac{1}{T-1} \left( \sum^T_{t=1} r_t^2 - 2T\bar{r}\left(\frac{1}{T} \sum^T_{t=1} r_t \right) + T \bar{r}^2 \right) \\
    &= \frac{1}{T-1} \left( \sum^T_{t=1} r_t^2 - T \bar{r}^2 \right) \\
    &= \frac{1}{T-1} \sum^T_{t=1} r_t^2 - \frac{T}{T-1} \left( \frac{1}{T}  \sum^T_{t=1} r_t \right)^2 \\
    &= \frac{1}{T-1} \sum^T_{t=1} r_t^2 - \frac{1}{T(T-1)} \left( \sum^T_{t=1} r_t \right)^2
\end{align*}

其中对数收益率与样本均值为：
\begin{equation*}
    r_t = \ln \left( \frac{S_t}{S_{t-1}} \right) \qquad \bar{r} = \frac{1}{T} \sum_{t=1}^{T}r_t
\end{equation*}
...
一种为特定针对隐含波动率，目的用于对比，对应的已实现波动率。即如在t时点，观察未来30天的隐含波动率，而后在t+30时间，观察已发生的30日之内的历史波动率。
...
具体而言，选取实际市场中观察到的一系列的$c(T,K)$与$K$，使用BS公式将其转换为其隐含波动率$\sigma$。随后在$(K_{\min},K_{\max})$中进行三次样条插值（Cubic spline interpolation），根据所需要的$K^{'}$得到插值后的$\sigma^{'}$。对于市场可观测行权价范围之外的，需要插值的$K'$，对于$K'<K_{\min}$则使用$\sigma' = \sigma_{K_{\min}}$。同理，对于$K'>K_{\max}$则使用$\sigma' = \sigma_{K_{\max}}$。最后，将所有插值得到的$K^{'}$与$\sigma^{'}$，通过BS进行计算，得到市场不能观察到的$c'(T,K^{'})$，进行积分最终得到第一项的离散化积分值。
\begin{equation*}
    \bar{c}_t(T,K) = \left\{
    \begin{array}{cl}
        ...
        p_t(T,K) + F_{t}^{*} B_t - K B_t &\quad V_\text{call} \leq \frac{1}{150\%} \cdot V_\text{put}
    \end{array}
    \right.
\end{equation*}
