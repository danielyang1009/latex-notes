...
\textbf{连续复利收益}（Continuously compounded return），或称为对数收益（Logarithmic return，或Log return）。

这里特指\textbf{收益}而非收益率，因为收益是指单期的，而收益率则可通过对单期收益率进行年化，或对多期收益率进行平均获得。记符号$r$为收益率（Rate of return）为将一段时间的收益（Return）$R$，转化为在标准化期限内的收益。对于标准化期限为\textbf{1年}的收益率，称为年化收益率（Annulized return）。
\begin{table}[H]
\centering
\begin{tabular}{@{}cll@{}}
\toprule
...
\begin{equation*}
    R_{pct} = e^{R_{log}} - 1 \quad\leftrightarrow\quad R_{log} = \ln (R_{pct} + 1)
\end{equation*}

\subsection{多期收益平均}

而计算\textbf{多期}的平均收益率，有\textbf{算术平均收益率}（Arithmetic mean rate of return），和\textbf{几何平均收益率}（Geometric mean rate of return）两种方式。假设共有$n$期，算术平均收益率有：
\begin{equation*}
...
\subsection{单期与多期收益率（年化）}

\subsubsection*{百分比收益率}

对于没有再投资（Reinvestment）的\textbf{单期}百分比年化收益率为（$\tau$单位为年）：
\begin{equation*}
    r_{pct} = \frac{R_{pct}}{\tau}
\end{equation*}

即如半年收益为$10\%$，则$1$年的年化为$20\%$。而对进行了再投资的百分比收益率为：
\begin{align*}
    1+R_{pct} &= (1+r_{pct})^\tau \\
    r_{pct} &= (1+R_{pct})^{\frac{1}{\tau}} - 1
\end{align*}

此时假设$R_{pct}$为$2$年单期收益，则$\tau = 2$，最终结果$r_{pct}$为将两年收益使用几何平均为年化百分比收益率。对于\textbf{多期}平均（几何平均）年化百分比收益率，假设R时间跨度为$1$年$k$期（若月收益，则有$k=12$），持有该资产的期限为$n$年，都应有：
\begin{equation*}
...
波动率的数学定义为：假设$r_t$为一个资产的收益率（一般使用对数收益率）时间序列，有$t=1,2,3,\dots,T$，则有样本方差为：
\begin{equation*}
