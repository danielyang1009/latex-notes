...
虽然一般将无模型波动率（Model-free implied volatility）认为是“BS隐含波动率的一种加权平均”更方便理解，但事实上无模型波动率与BS模型完全没有关系。虽然早期的VIX是由8个期权的隐含BS波动率加权得到的30天（日历日或22天交易日）波动率预期（Whaley (1993)），但现今已更新为无模型波动率（Demeterfi et al. (1999)）。且发布时是根据S\&P100（OEX），因为其交易量较大，而当时S\&P500（SPX）交易量只有其五分之一。
